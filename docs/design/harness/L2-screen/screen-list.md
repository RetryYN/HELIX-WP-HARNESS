---
layer: L2
sub_doc: screen-list
status: candidate_projection
parent_doc: docs/design/harness/L1-requirements/screen-requirements.md
source_authority: docs/requirements/l2/screen-list.md
source_sha256: 253468014e963c165725a8ef590f1630d12cdef3cd04abfd43ebde1f73502a6e
pair_artifact: docs/design/harness/L2-screen/wireframe.md
next_pair_freeze: L10
implemented_screens: ""
---

# HELIX L2 screen-list compatibility projection

HELIX reader用の非正本ID対応表。routeと状態契約の正本は`docs/requirements/l2/screen-list.md`であり、
このprojectionはG2 freezeまたは実装完了を表さない。

| HELIX reader ID | WP surface ID | WP source ID | route | authority |
| --- | --- | --- | --- | --- |
| PM-01 | WP-UI-01 | WP-SCR-01 | `/` | `docs/requirements/l2/screen-list.md` |
| PM-02 | WP-UI-02 | WP-SCR-02 | `/articles` | `docs/requirements/l2/screen-list.md` |
| PM-03 | WP-UI-03 | WP-SCR-03 | `/audit/clusters` | `docs/requirements/l2/screen-list.md` |
| PM-04 | WP-UI-04 | WP-SCR-04 | `/aio` | `docs/requirements/l2/screen-list.md` |
| PM-05 | WP-UI-05 | WP-SCR-05 | `/links` | `docs/requirements/l2/screen-list.md` |
| PM-06 | WP-UI-06 | WP-SCR-06 | `/rewrites` | `docs/requirements/l2/screen-list.md` |
| PM-07 | WP-UI-07 | WP-SCR-07 | `/outcomes` | `docs/requirements/l2/screen-list.md` |
| PM-08 | WP-UI-08 | WP-SCR-08 | `/calendar` | `docs/requirements/l2/screen-list.md` |

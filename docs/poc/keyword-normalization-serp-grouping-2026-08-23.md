...
- API: DataProviderB Google organic、standard queue、日本、ja、desktop
...
1. Excel のファイル digest・シート・行に結びつく `source_keyword_id` を DPB task の tag として渡し、
   raw 応答まで逆引きできた。
...
- main KW選定は derived_parent → 検索Vol最大（修飾語付きは除外）の順で決定するが、
  検索Volは一部KWのscript内ハードコードであり、Excel/DPB からの取り込みは未実装。
- coverage gateは「KWが存在する」最低線であり、自然さ、配置、過剰出現、意図充足は別ゲートとする。

## 修飾語ペアの追加実測

`it 就活サイト おすすめ` と `it 就活サイト 比較` を別のDPB standard taskとして取得し、

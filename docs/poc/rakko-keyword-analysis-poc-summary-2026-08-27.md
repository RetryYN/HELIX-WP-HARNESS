---
artifact_id: WP-POC-RAKKO-KEYWORD-ANALYSIS-20260827
status: primary-complete-unregistered
evidence_cutoff: 2026-08-27
source_branch: research/rakko-keyword-analysis
source_range: 2577737..66e106b
---

# Rakko キーワード分析 PoC — 一次要約（ハーネス再登録用）
...
| 種別 | 場所 | 概要 |
|---|---|---|
| 調査台帳 | `docs/research/rakko-keyword-competitive-analysis.md` | Rakko 公開機能・API と本リポ実装の差分台帳（根拠クラス付き） |
| API 契約 | `docs/research/rakko-openapi-inventory.json` / `evidence/rakko-openapi.json` | 公開 OpenAPI の全操作を証跡へ写像 |
| 能力判定 | `docs/research/rakko-capability-decisions.json` | 各能力の実装/不足/採否 |
| 料金 | `docs/research/rakko-pricing-policy.json` | 公開料金とプロバイダ費用の照合 |
| Web 機能棚卸 | `docs/research/rakko-web-capability-inventory.json` | 公開画面機能の一覧 |
...
1. Rakko 公開 API の全操作が、既存の SERP/GSC 証跡へ写像できる（`docs/research/rakko-openapi-inventory.json`）。
2. 有料プロバイダ（DFS）取得は料金検証つきのゲートで止められる（`gate provider plans with verified pricing`）。

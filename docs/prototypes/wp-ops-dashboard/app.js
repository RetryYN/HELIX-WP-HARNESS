...
function renderArticleQueries(){const rows=data.article_queries.filter((row)=>row.site_id===siteSelector.value);queryRowsRoot.innerHTML=rows.map((row)=>`<tr><td><strong>${row.title}</strong><span class="cell-note" title="${row.url}">${row.url}</span></td><td>${row.wp_article_id}</td><td><strong>${row.query}</strong></td><td>${yen.format(row.impressions)}</td><td>${yen.format(row.clicks)}</td><td>${Number(row.position).toFixed(2)}</td><td>過去${row.window_days}日<span class="cell-note">${new Date(row.observed_at).toLocaleString("ja-JP")}取得</span></td></tr>`).join("");document.querySelector("#query-empty").innerHTML=rows.length?"":`<strong>記事別クエリは未取得です</strong><span>GSCをpage × queryで取得すると表示されます。ページ別・クエリ別の独立集計は推測結合しません。</span>`}

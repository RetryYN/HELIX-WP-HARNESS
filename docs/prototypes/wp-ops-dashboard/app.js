...
function renderRows(){
  ...
  rowsRoot.innerHTML=rows.map((row)=>`<tr data-id="${row.group.id}"><td><strong>${row.keyword}</strong></td><td>${row.parent??"未確定"}</td><td><span class="relation ${row.relation}">${row.relation}</span></td><td>${row.group.category}</td><td>${row.group.main_keyword??"未確定"}</td><td>${row.group.state}</td><td>${row.group.wp_article_id??"—"}</td><td><button class="detail-button" type="button">詳細</button></td></tr>`).join("");

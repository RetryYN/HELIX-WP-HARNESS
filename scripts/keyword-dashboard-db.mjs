...
const schemaVersion = "keyword-dashboard.v21";
const replaceableSchemaVersions=new Set([schemaVersion,"keyword-dashboard.v20","keyword-dashboard.v19","keyword-dashboard.v18","keyword-dashboard.v17","keyword-dashboard.v16","keyword-dashboard.v15","keyword-dashboard.v14","keyword-dashboard.v13","keyword-dashboard.v12","keyword-dashboard.v11","keyword-dashboard.v10","keyword-dashboard.v9","keyword-dashboard.v8","keyword-dashboard.v7"]);
...
export function buildDashboardDb({ dbPath, fixturePath, artifactRoot, importedKeywords = [], gscEvidencePath, headingEvidencePath, competitorEvidencePath }) {
  ...
  db.exec(`
    ...
    DROP TABLE IF EXISTS competitor_task_terms; DROP TABLE IF EXISTS competitor_page_task_evidence;
    ...
    DROP TABLE IF EXISTS content_topic_coverage; DROP TABLE IF EXISTS content_generation_candidates;
    ...
    DROP TABLE IF EXISTS serp_feature_occurrences; DROP TABLE IF EXISTS serp_organic_attributes; DROP TABLE IF EXISTS serp_task_metadata; DROP TABLE IF EXISTS raw_snapshot_inventory;
    ...
    CREATE TABLE serp_feature_occurrences (feature_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), group_id TEXT NOT NULL REFERENCES keyword_groups(group_id), feature_type TEXT NOT NULL, occurrence_order INTEGER NOT NULL, rank_group INTEGER, rank_absolute INTEGER, page INTEGER, position TEXT, xpath TEXT, title TEXT, url TEXT, payload_json TEXT NOT NULL, UNIQUE(task_id,occurrence_order));
    CREATE TABLE serp_page_keyword_edges (task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), group_id TEXT NOT NULL REFERENCES keyword_groups(group_id), keyword TEXT NOT NULL, canonical_url TEXT NOT NULL, domain TEXT NOT NULL, rank INTEGER NOT NULL, PRIMARY KEY(task_id,canonical_url));
    CREATE TABLE simultaneous_keyword_relations (source_task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), target_task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), source_keyword TEXT NOT NULL, target_keyword TEXT NOT NULL, source_group_id TEXT NOT NULL REFERENCES keyword_groups(group_id), target_group_id TEXT NOT NULL REFERENCES keyword_groups(group_id), shared_url_count INTEGER NOT NULL, reciprocal_rank_score REAL NOT NULL, overlap_ratio REAL NOT NULL, same_group INTEGER NOT NULL, shared_urls_json TEXT NOT NULL, PRIMARY KEY(source_task_id,target_task_id));
    CREATE TABLE serp_page_coverage (canonical_url TEXT PRIMARY KEY, domain TEXT NOT NULL, keyword_count INTEGER NOT NULL, group_count INTEGER NOT NULL, best_rank INTEGER NOT NULL, rank_score REAL NOT NULL, top_task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), top_keyword TEXT NOT NULL);
    ...
    CREATE TABLE serp_ai_overviews (task_id TEXT PRIMARY KEY REFERENCES dfs_tasks(task_id), rank_absolute INTEGER NOT NULL, rank_group INTEGER NOT NULL, asynchronous INTEGER NOT NULL, markdown TEXT, items_json TEXT NOT NULL, references_json TEXT NOT NULL);
    CREATE TABLE aio_citation_references (citation_id TEXT PRIMARY KEY, task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), group_id TEXT NOT NULL REFERENCES keyword_groups(group_id), source_keyword TEXT NOT NULL, reference_order INTEGER NOT NULL, domain TEXT NOT NULL, url TEXT NOT NULL, canonical_url TEXT, source TEXT, title TEXT, text TEXT, position TEXT, is_own_domain INTEGER NOT NULL, organic_url_rank INTEGER, organic_domain_rank INTEGER, UNIQUE(task_id,reference_order));
    ...
    CREATE TABLE competitor_page_task_evidence (task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), page_id TEXT NOT NULL REFERENCES competitor_pages(page_id), best_rank INTEGER NOT NULL, PRIMARY KEY(task_id,page_id));
    ...
    CREATE TABLE competitor_task_terms (task_id TEXT NOT NULL REFERENCES dfs_tasks(task_id), term TEXT NOT NULL, page_count INTEGER NOT NULL, total_count INTEGER NOT NULL, title_count INTEGER NOT NULL, heading_count INTEGER NOT NULL, title_page_count INTEGER NOT NULL, heading_page_count INTEGER NOT NULL, weighted_score REAL NOT NULL, evidence_page_ids_json TEXT NOT NULL, PRIMARY KEY(task_id,term));
  ...
  const insertRawSnapshot=db.prepare("INSERT INTO raw_snapshot_inventory VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
  for(const snapshot of snapshots.values()){const groupId=selectedTaskGroups.get(snapshot.task_id)??null;insertRawSnapshot.run(snapshot.task_id,snapshot.keyword,snapshot.artifact_dataset,snapshot.snapshot_path,snapshot.snapshot_digest,snapshot.observed_at,snapshot.cost,JSON.stringify(snapshot.task_metadata.item_types),groupId==null?"unconnected":"connected",groupId,groupId==null?"current analysis fixture does not reference this independently acquired PoC task":null)}
  ...
  const pageKeywordGraph=buildSerpPageKeywordGraph(db.prepare("SELECT task_id,group_id,keyword FROM dfs_tasks").all(),db.prepare("SELECT task_id,rank_group,url FROM serp_organic_results").all(),{depth:10});
  ...
  const insertGeneration=db.prepare("INSERT INTO content_generation_candidates VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
  for(const candidate of buildEvidenceBoundGenerationCandidates(fixture.groups,topicProposals,termsByGroup))insertGeneration.run(candidate.candidate_id,candidate.group_id,candidate.content_type,candidate.heading_level,candidate.text,candidate.evidence_type,JSON.stringify(candidate.evidence_ids),JSON.stringify(candidate.coverage),candidate.status,candidate.candidate_digest);
...
export function projectDashboard(db) {
  ...
  const serpTaskMetadata=db.prepare("SELECT m.*,d.cost,d.snapshot_path,d.snapshot_digest,d.observed_at FROM serp_task_metadata m JOIN dfs_tasks d USING(task_id) ORDER BY m.task_id").all().map(({path_json,task_data_json,spell_json,refinement_chips_json,item_types_json,...row})=>({...row,path:JSON.parse(path_json),task_data:JSON.parse(task_data_json),spell:spell_json==null?null:JSON.parse(spell_json),refinement_chips:refinement_chips_json==null?null:JSON.parse(refinement_chips_json),item_types:JSON.parse(item_types_json)}));
  ...
  const serpFeatureSummary=db.prepare("SELECT feature_type,COUNT(*) AS occurrence_count,COUNT(DISTINCT task_id) AS task_count FROM serp_feature_occurrences GROUP BY feature_type ORDER BY occurrence_count DESC,feature_type").all(),serpSpecialFeatures=db.prepare("SELECT * FROM serp_feature_occurrences WHERE feature_type NOT IN ('people_also_ask','related_searches','ai_overview') ORDER BY task_id,occurrence_order").all().map(({payload_json,...row})=>({...row,payload:JSON.parse(payload_json)}));
  const serpPageKeywordEdges=db.prepare("SELECT * FROM serp_page_keyword_edges ORDER BY task_id,rank,canonical_url").all();
  ...
  const contentGenerationCandidates=db.prepare("SELECT * FROM content_generation_candidates ORDER BY group_id,content_type,candidate_id").all().map(({evidence_ids_json,coverage_json,...row})=>({...row,evidence_ids:JSON.parse(evidence_ids_json),coverage:JSON.parse(coverage_json)}));
  ...
  const competitorSummary={page_count:competitorPages.length,successful_page_count:competitorPages.filter((page)=>page.status==="ok").length,heading_count:competitorHeadings.length,term_count:Number(db.prepare("SELECT COUNT(*) AS count FROM competitor_terms").get().count),task_term_count:Number(db.prepare("SELECT COUNT(*) AS count FROM competitor_task_terms").get().count),projected_term_count:competitorTerms.length,group_count:Number(db.prepare("SELECT COUNT(DISTINCT group_id) AS count FROM competitor_page_group_evidence").get().count),task_count:Number(db.prepare("SELECT COUNT(DISTINCT task_id) AS count FROM competitor_page_task_evidence").get().count)};
  return { generated_at: metadata.generated_at, sites, groups, keyword_inventory: db.prepare("SELECT * FROM imported_keywords ORDER BY site_id, source_sheet, source_row").all(), keyword_hierarchy:keywordHierarchy, normalization_aliases: JSON.parse(metadata.normalization_aliases), raw_snapshot_inventory:rawSnapshotInventory, serp_task_metadata:serpTaskMetadata, serp_feature_summary:serpFeatureSummary, serp_special_features:serpSpecialFeatures, serp_demand_occurrences:demandOccurrences, serp_demands:serpDemands, serp_organic_results:serpOrganicResults, serp_page_keyword_edges:serpPageKeywordEdges, simultaneous_keyword_relations:simultaneousKeywordRelations, serp_page_coverage:serpPageCoverage, serp_domain_coverage:serpDomainCoverage, serp_ai_overviews:serpAiOverviews, aio_citation_references:aioCitationReferences, aio_citation_domains:aioCitationDomains, aio_content_elements:aioContentElements, content_topic_proposals:contentTopicProposals, content_topic_coverage:contentTopicCoverage, content_structure_candidates:contentStructureCandidates, content_generation_candidates:contentGenerationCandidates, competitor_summary:competitorSummary, competitor_pages:competitorPages, competitor_page_evidence:competitorPageEvidence, competitor_page_task_evidence:competitorPageTaskEvidence, competitor_headings:competitorHeadings, competitor_terms:competitorTerms, competitor_task_terms:competitorTaskTerms, article_links: db.prepare("SELECT * FROM article_links ORDER BY link_id").all(), article_queries: articleQueries, article_query_summaries: articleQuerySummaries, primary_query_ranking: Object.fromEntries(Object.entries(rankingBySite).map(([siteId,ranking])=>[siteId,{...ranking,method:"log_impressions_plus_clicks_at_p95"}])), gsc_articles: gscArticles.map(({category_ids_json,headings_json,...article})=>article) };
}

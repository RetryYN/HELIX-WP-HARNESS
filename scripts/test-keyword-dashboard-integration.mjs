...
const failedSnippetPage={...competitorPage("f",1),url:failedSnippetUrl,domain:new URL(failedSnippetUrl).hostname,status:"robots_denied",http_status:null,content_type:null,final_url:null,title:null,snapshot_path:null,snapshot_digest:null,text_length:null,text_digest:null,headings:[],terms:[]};
writeFileSync(competitorManifestPath,JSON.stringify({schema_version:"competitor-content-evidence.v2",parser_version:"competitor-content-core.v2",generated_at:"2026-08-26T00:00:00.000Z",selection:{max_rank:3,page_limit:3,selected_count:3},pages:[competitorPage("a",1),competitorPage("d",2),failedSnippetPage]}));
...
assert.equal(competitorDb.prepare("SELECT COUNT(*) AS count FROM competitor_page_terms").get().count,4);assert.deepEqual(competitorDb.prepare("SELECT count,title_count,heading_count,in_title,in_heading FROM competitor_page_terms WHERE term='構成' ORDER BY page_id").all().map((row)=>({...row})),[{count:4,title_count:1,heading_count:1,in_title:1,in_heading:1},{count:4,title_count:1,heading_count:1,in_title:1,in_heading:1}]);
...
assert.equal(competitorDb.prepare("SELECT COUNT(*) AS count FROM competitor_page_task_evidence").get().count,3);assert.equal(competitorDb.prepare("SELECT page_count FROM competitor_task_terms WHERE task_id=? AND term='構成'").get(competitorTask).page_count,2);
assert.equal(competitorDb.prepare("SELECT COUNT(*) AS count FROM competitor_serp_snippet_evidence WHERE length(evidence_digest)=64").get().count,1);
const competitorProjection=projectDashboard(competitorDb);assert.equal(competitorProjection.competitor_pages.length,3);assert.deepEqual(competitorProjection.competitor_page_evidence[0].task_ids,[competitorTask]);assert.equal(competitorProjection.competitor_terms[0].evidence_page_ids.length,2);assert.equal(competitorProjection.competitor_task_terms[0].evidence_page_ids.length,2);assert.equal(competitorProjection.competitor_serp_snippet_evidence.length,1);
...
assert.ok(competitorProjection.content_generation_candidates.every((item)=>item.review&&item.review.review_digest.length===64));assert.ok(competitorProjection.content_generation_candidates.every((item)=>["ready","needs_review","blocked"].includes(item.review.review_state)));assert.ok(competitorProjection.content_generation_candidates.every((item)=>item.review.review_policy==="evidence-bound-review.v3"&&typeof item.review.oracle.evidence_reference_resolved==="boolean"));
assert.ok(competitorProjection.content_generation_candidates.every((item)=>item.generation?.generator_kind==="deterministic_rule"&&item.generation.input_digest.length===64));assert.ok(competitorProjection.content_generation_candidates.some((item)=>item.generation.variant_key.endsWith(":demand_explainer")));
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM raw_snapshot_reuse_audit").get().count,110,"every retained snapshot must receive an explicit reuse disposition");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM raw_snapshot_reuse_audit WHERE reuse_state='historical_comparable'").get().count,2,"same-keyword historical snapshots must be connected as comparisons");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM raw_snapshot_reuse_audit WHERE reuse_state='adjacent_intent_candidate'").get().count,2,"adjacent intents without source identity must remain review candidates");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM raw_snapshot_reuse_audit WHERE reuse_state='out_of_scope'").get().count,6,"different corpus snapshots must remain isolated");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_snapshot_organic_observations").get().count,1016,"organic rows from all retained snapshots must remain queryable");
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_action_signals").get().count,25,"observed spelling, special features, organic video attributes and commerce evidence must become proposed actions");
...
assert.equal(pocDb.prepare("SELECT SUM(video_result_count) AS count FROM serp_action_signals").get().count,19,"all organic is_video observations must reach the action layer");assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_action_signals WHERE video_result_count>0 AND evidence_json NOT LIKE '%organic_result_attribute%'").get().count,0,"video actions must retain rank-level attribute evidence");
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_intent_fingerprints").get().count,100,"every retained task must have one composite intent fingerprint");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_intent_pair_reviews").get().count,356,"review and bounded related-intent pairs must be retained");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_intent_pair_reviews WHERE decision='merge_review' AND review_required=1").get().count,21,"composite evidence must expose review-only merge candidates");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_intent_pair_reviews WHERE auto_mutation!=0").get().count,0,"intent analysis must never mutate groups automatically");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_boundary_reviews").get().count,68,"actionable URL/intent boundary reviews must be retained once per task pair");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_boundary_reviews WHERE decision='merge_consensus_review'").get().count,1,"only URL and composite-intent consensus may remain a merge candidate");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_boundary_reviews WHERE auto_mutation!=0 OR left_task_id='' OR right_task_id=''").get().count,0,"boundary reviews retain task identity and never auto-mutate groups");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_depth_stability").get().count,68,"every actionable boundary pair must retain top3/5/10 sensitivity evidence");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_depth_stability WHERE stability_state='threshold_flip'").get().count,51,"depth-sensitive threshold flips must remain explicit");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM serp_depth_stability WHERE robust_merge=1 AND stability_state='stable_merge'").get().count,1,"only a cross-depth stable consensus may be marked robust merge");
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_structures WHERE site_id='it-shukatu.com'").get().count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_observed_links WHERE site_id='it-shukatu.com'").get().count,2539);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_observed_links WHERE site_id='it-shukatu.com' AND is_internal=1").get().count,2061);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_images WHERE site_id='it-shukatu.com'").get().count,1222);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_sections WHERE site_id='it-shukatu.com'").get().count,1499);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_paragraphs WHERE site_id='it-shukatu.com'").get().count,8050);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_page_seo_metadata WHERE site_id='it-shukatu.com'").get().count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_page_seo_metadata WHERE error IS NULL AND http_status=200 AND length(evidence_digest)=64").get().count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_page_seo_metadata WHERE canonical_url IS NULL OR description IS NULL OR title='' OR head_digest IS NULL").get().count,0,"all public pages must retain core SEO head evidence without retaining HTML");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM site_public_urls WHERE site_id='it-shukatu.com'").get().count,70);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM site_public_urls WHERE surface_type='post'").get().count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM site_public_urls WHERE surface_type='page'").get().count,9);assert.equal(pocDb.prepare("SELECT url_count FROM site_surface_runs WHERE site_id='it-shukatu.com'").get().url_count,70);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM public_page_surface_observations WHERE site_id='it-shukatu.com' AND http_status=200 AND error IS NULL").get().count,70);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM public_page_surface_links WHERE site_id='it-shukatu.com'").get().count,12586);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM public_page_surface_links WHERE site_id='it-shukatu.com' AND is_internal=1").get().count,11707);
const retainedCompetitorManifest=JSON.parse(readFileSync(path.resolve(".helix/evidence/competitor-content/manifest.json"),"utf8")),retainedCompetitorTermCount=retainedCompetitorManifest.pages.reduce((sum,page)=>sum+(page.terms?.length??0),0);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM competitor_page_terms").get().count,retainedCompetitorTermCount,"all terms in the current content evidence manifest must reach SQLite");assert.ok(retainedCompetitorTermCount>200000,"the full top-10 competitor corpus must be present rather than a small fixture");
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  assert.equal(actual.sites[0].data_disposition.length,16);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="nested_serp_feature_items").observed_count,20);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="nested_serp_feature_items").delta_count,0);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_cooccurrence_projection").observed_count,pocDb.prepare("SELECT COUNT(*) count FROM competitor_terms").get().count);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_cooccurrence_projection").delta_count,0);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="wordpress_paragraph_text").state,"intentionally_not_retained");assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="keyword_serp").delta_count,10594);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="gsc_normalization").delta_count,3);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").delta_count,29);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").state,"partial_with_serp_fallback");assert.equal(actual.competitor_summary.failed_page_with_serp_snippet_count,29);assert.equal(actual.competitor_summary.serp_snippet_observation_count,925);assert.equal(actual.competitor_serp_snippet_evidence.filter((row)=>actual.competitor_pages.find((page)=>page.page_id===row.page_id)?.status!=="ok").length,36);assert.ok(actual.competitor_serp_snippet_evidence.every((row)=>row.evidence_digest.length===64&&row.source_snapshot_digest.length===64));assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="paa_answers").observed_count,0);assert.ok(actual.sites[0].data_disposition.every((row)=>row.evidence_digest.length===64));
  assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="wordpress_content_structure").state,"retained_digest_only");assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="wordpress_public_seo_metadata").delta_count,0);assert.equal(actual.sites[0].wp_page_seo_metadata.length,59);assert.ok(actual.sites[0].wp_page_seo_metadata.every((row)=>row.canonical_url&&row.description&&row.evidence_digest.length===64));assert.equal(actual.wp_content_structures.length,59);assert.equal(actual.wp_observed_links.length,2539);assert.ok(actual.wp_observed_links.every((row)=>row.evidence_digest.length===64));assert.equal(actual.wp_content_images.length,1222);assert.equal(actual.wp_content_sections.length,1499);
  assert.equal(actual.sites[0].wp_page_seo_audits.length,59);assert.equal(actual.sites[0].wp_page_seo_audits.filter((row)=>row.state==="critical").length,0);assert.equal(actual.sites[0].wp_page_seo_audits.filter((row)=>row.state==="warning").length,0);assert.equal(actual.sites[0].wp_page_seo_audits.filter((row)=>row.state==="informational").length,59);assert.ok(actual.sites[0].wp_page_seo_audits.every((row)=>row.findings.some((item)=>item.code==="json_ld_not_observed")&&row.audit_digest.length===64));
  assert.equal(actual.wp_paragraph_summaries.length,59);assert.equal(actual.wp_paragraph_summaries.reduce((sum,row)=>sum+row.paragraph_count,0),8050);assert.equal(actual.wp_paragraph_summaries.reduce((sum,row)=>sum+row.p_count,0),6204);assert.equal(actual.wp_paragraph_summaries.reduce((sum,row)=>sum+row.li_count,0),1798);
  assert.deepEqual({...actual.sites[0].content_generation_quality_oracle,issue_counts:undefined,oracle_digest:undefined},{policy:"evidence-bound-review.v3",candidate_count:765,title_count:185,heading_count:580,ready_count:723,needs_review_count:37,blocked_count:5,evidence_resolved_count:765,competitor_copy_risk_count:3,existing_collision_count:1,issue_counts:undefined,oracle_digest:undefined});assert.equal(actual.sites[0].content_generation_quality_oracle.oracle_digest.length,64);assert.equal(actual.sites[0].content_generation_quality_oracle.issue_counts.cross_group_candidate_duplicate,2);assert.equal(actual.sites[0].content_generation_quality_oracle.issue_counts.serp_feature_observation_review,15);assert.equal(actual.sites[0].content_generation_quality_oracle.issue_counts.title_axis_overload??0,0);assert.ok(actual.content_generation_candidates.every((row)=>row.review.oracle.evidence_reference_resolved&&row.review.review_policy==="evidence-bound-review.v3"&&row.generation.generator_version==="evidence-bound-generation.v4"));const featureCandidates=actual.content_generation_candidates.filter((row)=>row.evidence_type==="serp_feature_item");assert.equal(featureCandidates.length,15);assert.equal(featureCandidates.filter((row)=>row.content_type==="title").length,6);assert.equal(featureCandidates.filter((row)=>row.content_type==="heading").length,9);assert.ok(featureCandidates.every((row)=>row.status==="proposed"&&row.review.review_state==="needs_review"&&row.coverage.review_required&&row.generation.generator_version==="evidence-bound-generation.v4"));assert.ok(actual.content_generation_candidates.filter((row)=>row.content_type==="title"&&row.coverage.axes).every((row)=>row.coverage.axes.length<=3&&!/[?？]/u.test(row.coverage.axes.join(""))));assert.deepEqual(new Set(featureCandidates.flatMap((row)=>row.evidence_ids)),new Set(actual.serp_special_features.flatMap((row)=>row.items).map((row)=>row.feature_item_id)));assert.equal(actual.content_generation_candidates.filter((row)=>row.generation.variant_key==="competitor_heading_fallback").length,14);assert.equal(actual.content_generation_candidates.filter((row)=>row.coverage.topic_assignment_policy==="source_task_fallback").length,10);assert.equal(actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="brief_ready").length,63);assert.deepEqual(actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="blocked").map((row)=>row.group_id),[]);
  assert.equal(actual.content_outlines.length,63);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.candidate_count,0),580);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.selected_count,0),510);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.h2_count,0),251);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.h3_count,0),259);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.unassigned_candidates.length,0),63);assert.equal(actual.content_outlines.flatMap((row)=>row.sections).filter((row)=>row.evidence_type==="serp_feature_item").length,7);assert.ok(actual.content_outlines.every((row)=>row.policy==="evidence-outline-selection.v1"&&!row.auto_approval));assert.ok(actual.content_outlines.flatMap((row)=>row.sections.flatMap((section)=>section.children)).every((child)=>child.parent_relation.parent_candidate_id&&child.parent_relation.policy==="shared-evidence-then-character-trigram.v1"&&(child.parent_relation.shared_evidence_count>0||child.parent_relation.lexical_jaccard>0||child.parent_relation.substring_match)));
  const compositions=actual.content_structure_candidates.map((row)=>row.composition);assert.equal(compositions.length,63);assert.equal(compositions.filter((row)=>row.review_state==="ready").length,48);assert.equal(compositions.filter((row)=>row.review_state==="needs_review").length,15);assert.equal(compositions.filter((row)=>row.review_state==="blocked").length,0);assert.equal(compositions.filter((row)=>row.metrics.selection_changed).length,22);assert.equal(compositions.filter((row)=>row.issues.includes("title_outline_lexical_gap")).length,0);assert.equal(compositions.reduce((sum,row)=>sum+row.metrics.selected_heading_count,0),510);assert.equal(compositions.reduce((sum,row)=>sum+row.metrics.shared_evidence_count,0),166);assert.equal(compositions.reduce((sum,row)=>sum+row.metrics.selected_heading_review_required_count,0),19);assert.ok(compositions.every((row)=>row.policy==="content-plan-coherence.v2"&&row.status==="proposed"&&!row.auto_approval&&row.title_candidate_id&&row.baseline_title_candidate_id&&row.composition_digest.length===64));assert.ok(compositions.every((row)=>row.metrics.title_quality_delta===0&&row.metrics.title_evidence_coverage_delta>=0&&row.metrics.title_outline_lexical_coverage_delta>=0));assert.ok(compositions.every((row)=>actual.content_generation_candidates.some((candidate)=>candidate.candidate_id===row.title_candidate_id)&&row.selected_heading_ids.every((id)=>actual.content_generation_candidates.some((candidate)=>candidate.candidate_id===id))));
  assert.equal(actual.sites[0].public_surface_inventory.length,70);assert.equal(actual.sites[0].public_surface_inventory.filter((row)=>row.analysis_state==="post_analysis_connected").length,59);assert.equal(actual.sites[0].public_surface_inventory.filter((row)=>row.analysis_state==="surface_connected_by_public_navigation").length,2);assert.equal(actual.sites[0].public_surface_inventory.filter((row)=>row.analysis_state==="surface_unlinked_in_observed_documents").length,9);assert.equal(actual.sites[0].public_surface_inventory.filter((row)=>row.redirect_observed).length,4);assert.equal(actual.sites[0].public_surface_inventory.filter((row)=>/noindex/iu.test(row.robots??"")).length,5);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="sitemap_public_url_surface").delta_count,11);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="public_document_navigation").delta_count,0);assert.equal(actual.sites[0].public_surface_run.url_count,70);
  assert.equal(actual.internal_link_opportunities.length,17);assert.ok(actual.internal_link_opportunities.every((row)=>row.state==="proposed"&&row.evidence_digest.length===64&&row.evidence_relation_ids.length>0));assert.ok(actual.internal_link_opportunities.every((row)=>!actual.wp_observed_links.some((link)=>link.source_wp_article_id===row.source_wp_article_id&&link.target_wp_article_id===row.target_wp_article_id)),"observed article links must never be proposed again");
  assert.ok(actual.content_structure_candidates.every((row)=>row.internal_link_plan.plan_digest.length===64));assert.equal(actual.content_structure_candidates.reduce((sum,row)=>sum+row.internal_link_plan.outgoing.length,0),17,"every link opportunity must return to exactly one source content brief");
  ...
  assert.deepEqual(actual.sites[0].snapshot_reuse_audit.summary,{snapshot_count:104,current_analysis_count:100,historical_comparison_count:2,adjacent_candidate_count:2,out_of_scope_count:6,external_acquisition_triggered:false});assert.equal(actual.sites[0].snapshot_reuse_audit.comparisons.length,2);assert.ok(actual.sites[0].snapshot_reuse_audit.comparisons.every((row)=>row.contract_match&&row.shared_url_count===9&&row.evidence_digest.length===64));assert.ok(actual.sites[0].data_disposition.some((row)=>row.dataset==="raw_serp_snapshots"&&row.observed_count===102&&row.delta_count===8));
  ...
  assert.equal(actual.serp_action_signals.length,25);assert.ok(actual.serp_action_signals.some((row)=>row.signal_types.includes("visual")&&row.recommended_formats.includes("original_images")));assert.ok(actual.serp_action_signals.some((row)=>row.signal_types.includes("commercial")&&row.rated_result_count===2));assert.ok(actual.serp_action_signals.some((row)=>row.signal_types.includes("spelling")&&row.corrected_keyword==="就活ネクタイ"));assert.equal(actual.serp_action_signals.reduce((sum,row)=>sum+row.video_result_count,0),19);assert.equal(actual.serp_action_signals.filter((row)=>row.signal_types.includes("video")).length,20);const actionFeatureItems=actual.serp_action_signals.flatMap((row)=>row.evidence.filter((evidence)=>evidence.evidence_type==="serp_feature_item"));assert.equal(actionFeatureItems.length,20);assert.equal(new Set(actionFeatureItems.map((evidence)=>evidence.evidence_id)).size,20);assert.ok(actual.serp_action_signals.some((row)=>row.heading_guidance.some((guidance)=>guidance.includes("Knowledge Graph出典"))));assert.ok(actual.serp_action_signals.every((row)=>row.status==="proposed"&&row.evidence.length>0));
  assert.equal(actual.sites[0].serp_brand_analysis.summary.brand_count,260);assert.equal(actual.sites[0].serp_brand_analysis.summary.domain_count,226);assert.equal(actual.sites[0].serp_brand_analysis.summary.platform_publisher_count,38);assert.equal(actual.sites[0].serp_brand_analysis.summary.multi_domain_brand_count,2);assert.ok(actual.sites[0].serp_brand_analysis.brands.every((row)=>row.evidence_digest.length===64));
  ...
  assert.equal(actual.serp_ai_overviews.filter((row)=>row.response_state==="resolved"&&row.analysis_eligible&&!row.retrieval_required).length,17);assert.equal(actual.serp_ai_overviews.filter((row)=>row.response_state==="async_pending"&&!row.analysis_eligible&&row.retrieval_required).length,51);assert.equal(actual.sites[0].aio_acquisition_summary.resolved_count,17);assert.equal(actual.sites[0].aio_acquisition_summary.async_pending_count,51);assert.ok(actual.sites[0].data_disposition.some((row)=>row.dataset==="aio_answer_payloads"&&row.observed_count===17&&row.delta_count===51));
  assert.equal(actual.content_structure_candidates.filter((item)=>item.draft_package).length,63);assert.ok(actual.content_structure_candidates.every((item)=>item.draft_package.package_version==="content-draft-package.v3"&&item.draft_package.input.outline_policy==="evidence-outline-selection.v1"));assert.ok(actual.content_structure_candidates.every((item)=>item.draft_package.body_state==="generated"));assert.ok(actual.content_structure_candidates.every((item)=>item.draft_package.input_digest.length===64&&item.draft_package.package_digest.length===64));assert.ok(actual.content_structure_candidates.every((item)=>item.draft_package.draft_revision?.renderer_version==="content-evidence-draft.v1"&&item.draft_package.draft_revision.review.publication_state==="blocked"&&!item.draft_package.draft_revision.review.auto_approval&&item.draft_package.draft_revision.text&&item.draft_package.draft_revision.html&&item.draft_package.draft_revision.evidence_oracle.all_references_resolved&&item.draft_package.draft_revision.evidence_oracle.fact_verification_state==="pending_primary_source"&&item.draft_package.draft_revision.claim_evidence.every((reference)=>reference.resolution_state==="resolved"&&reference.source)));assert.ok(actual.content_structure_candidates.flatMap((item)=>item.draft_package.input.headings.filter((heading)=>heading.level===3)).every((heading)=>heading.parent_candidate_id&&heading.parent_relation.policy==="shared-evidence-then-character-trigram.v1"&&(heading.parent_relation.shared_evidence_count>0||heading.parent_relation.lexical_jaccard>0||heading.parent_relation.substring_match)));assert.ok(actual.content_structure_candidates.some((item)=>item.draft_package.input.serp_action_signals.some((signal)=>signal.evidence.some((evidence)=>evidence.evidence_type==="serp_feature_item"))));assert.ok(actual.content_structure_candidates.some((item)=>item.draft_package.gates.some((gate)=>gate.gate==="citation_approval"&&gate.status==="pending")));
  const draftCitationRecommendations=actual.content_structure_candidates.flatMap((item)=>item.draft_package.draft_revision.citation_recommendations);assert.equal(draftCitationRecommendations.length,311);assert.equal(new Set(draftCitationRecommendations.map((row)=>row.claim_id)).size,107);assert.equal(new Set(draftCitationRecommendations.map((row)=>row.url)).size,40);assert.ok(draftCitationRecommendations.every((row)=>row.recommendation_state==="proposed"&&row.review_state==="needs_review"&&row.approval_state==="unreviewed"&&!row.auto_approval));
...
const mcpTools=await fetch(`http://127.0.0.1:${port}/mcp`,{method:"POST",headers:{"content-type":"application/json","mcp-protocol-version":"2025-06-18"},body:JSON.stringify({jsonrpc:"2.0",id:2,method:"tools/list"})});assert.equal(mcpTools.status,200);const mcpToolList=(await mcpTools.json()).result.tools;assert.equal(mcpToolList.length,18);assert.deepEqual(new Set(mcpToolList.map((tool)=>tool.name)),new Set(["search_keywords","search_demands","list_question_candidates","get_content_brief","get_acquisition_status","analyze_titles","analyze_headings","list_outlines","review_content_compositions","read_content_drafts","analyze_competitor_domains","inspect_serp_results","track_rank_history","get_market_data","search_serp_feature_items","review_serp_intent_pairs","review_keyword_boundaries","review_serp_depth_stability"]));
...
for(const testSite of first.sites){const scopedTaskIds=new Set(first.groups.filter((row)=>row.site_id===testSite.site_id).flatMap((row)=>row.task_ids)),scopedEdges=first.serp_page_keyword_edges.filter((row)=>scopedTaskIds.has(row.task_id)),expectedUrls=new Set(scopedEdges.map((row)=>row.canonical_url)),expectedDomains=new Set(scopedEdges.map((row)=>row.domain)),response=await fetch(`http://127.0.0.1:${port}/api/v1/pages?site_id=${encodeURIComponent(testSite.site_id)}&limit=100`),payload=await response.json();assert.equal(response.status,200);assert.equal(payload.meta.total,expectedUrls.size,`page coverage must be recomputed inside ${testSite.site_id}`);assert(payload.data.every((row)=>expectedUrls.has(row.canonical_url)&&row.scope.site_id===testSite.site_id),`page API must not leak another site's URLs into ${testSite.site_id}`);const domainResponse=await fetch(`http://127.0.0.1:${port}/api/v1/domains?site_id=${encodeURIComponent(testSite.site_id)}&limit=100`),domainPayload=await domainResponse.json();assert.equal(domainPayload.meta.total,expectedDomains.size,`domain coverage must be recomputed inside ${testSite.site_id}`);assert(domainPayload.data.every((row)=>expectedDomains.has(row.domain)&&row.scope.site_id===testSite.site_id),`domain API must not leak another site's metrics into ${testSite.site_id}`)}
...
assert.match(app,/renderBrandAnalysis/);assert.match(app,/serp_brand_analysis/);assert.match(html,/SERP表示ブランド占有/);assert.match(html,/brand-rows/);
...
assert.match(app,/aio_acquisition_summary/);assert.match(app,/非同期未回収/);assert.match(html,/回答payload取得済み17件/);
...
assert.match(app,/site\.dfs_enrichment_status/);assert.match(app,/data\.keyword_market_metrics\.filter\(\(row\)=>row\.site_id===siteId\)/);assert.match(app,/data\.keyword_monthly_searches\.filter\(\(row\)=>row\.site_id===siteId\)/);assert.match(app,/data\.domain_ranked_keywords\.filter\(\(row\)=>row\.site_id===siteId\)/);assert.match(app,/追加市場データは未取得です/);assert.match(app,/renderMarketData/);
...
assert.match(app,/helix-seo-research-history\.v1/);assert.match(app,/localStorage/);assert.match(app,/searchParams\.set\("view"/);assert.match(html,/ブラウザ内検索履歴/);assert.match(html,/履歴を全消去/);
assert.match(app,/wp_observed_links/);assert.match(app,/wp_content_structures/);assert.match(app,/記事間解決/);assert.match(html,/本文を保存せず抽出した実測リンク/);
assert.match(app,/internal_link_opportunities/);assert.match(app,/source_section_candidate/);assert.match(html,/根拠付き内部リンク候補/);assert.match(html,/自動追加はしません/);
assert.match(app,/internal_link_plan/);assert.match(app,/内部リンクbrief/);assert.match(app,/既存実測linkは除外済み/);
assert.match(app,/renderSeoAudit/);assert.match(app,/wp_page_seo_audits/);assert.match(app,/audit_digest/);assert.match(html,/公開SEO head監査/);assert.match(html,/canonical・indexability/);
assert.match(app,/renderSurfaceInventory/);assert.match(app,/public_surface_inventory/);assert.match(app,/surface_connected_by_public_navigation/);assert.match(app,/document_incoming_source_count/);assert.match(html,/sitemap公開URL母集団/);assert.match(html,/固定ページ・HTML sitemap/);

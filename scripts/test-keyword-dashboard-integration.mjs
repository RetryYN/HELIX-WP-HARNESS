...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  const compositions=actual.content_structure_candidates.map((row)=>row.composition);assert.equal(compositions.length,63);assert.equal(compositions.filter((row)=>row.review_state==="ready").length,33);assert.equal(compositions.filter((row)=>row.review_state==="needs_review").length,30);assert.equal(compositions.filter((row)=>row.review_state==="blocked").length,0);assert.equal(compositions.reduce((sum,row)=>sum+row.metrics.selected_heading_count,0),510);assert.equal(compositions.reduce((sum,row)=>sum+row.metrics.shared_evidence_count,0),248);assert.equal(compositions.reduce((sum,row)=>sum+row.metrics.selected_heading_review_required_count,0),19);assert.ok(compositions.every((row)=>row.policy==="content-plan-coherence.v1"&&row.status==="proposed"&&!row.auto_approval&&row.title_candidate_id&&row.composition_digest.length===64));assert.ok(compositions.every((row)=>actual.content_generation_candidates.some((candidate)=>candidate.candidate_id===row.title_candidate_id)&&row.selected_heading_ids.every((id)=>actual.content_generation_candidates.some((candidate)=>candidate.candidate_id===id))));

...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_topology_reviews").get().count,24,"task evidence must collapse into unique content group-pair reviews");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_topology_reviews WHERE topology_decision='consolidation_review'").get().count,1,"the robust merge must become one content consolidation review");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_topology_reviews WHERE topology_decision='depth_sensitive_hold'").get().count,21,"depth-sensitive group pairs must block topology changes");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_topology_reviews WHERE auto_mutation!=0").get().count,0,"content topology never auto-mutates groups or articles");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_consolidation_blueprints").get().count,1,"the robust consolidation review must have one title/heading diff blueprint");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_consolidation_blueprints WHERE source_heading_count=10 AND duplicate_heading_pair_count=3 AND unique_heading_count=4 AND projected_merged_heading_count=7").get().count,1,"the blueprint must retain the complete 10-to-7 heading consolidation diff");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_consolidation_blueprints WHERE title_selection_state!='unresolved_not_auto_selected' OR auto_mutation!=0").get().count,0,"content consolidation never chooses a survivor or mutates content automatically");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_consolidation_blueprints WHERE recommended_survivor_group_id='it-shukatu-serp-012' AND recommended_duplicate_representative_count=3 AND survivor_recommendation_state='review_only_recommendation'").get().count,1,"the decision packet must rank the evidence-strong survivor and all duplicate representatives for review");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_consolidation_blueprints WHERE json_extract(merged_outline_gate_json,'$.state')='ready_for_editor_review' AND json_extract(merged_outline_gate_json,'$.heading_count')=7 AND json_array_length(merged_outline_preview_json)=4").get().count,1,"the merged outline must retain four ordered H2 sections, seven total headings and a passing structure gate");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_consolidation_blueprints WHERE json_extract(merged_outline_gate_json,'$.source_evidence_count')=21 AND json_extract(merged_outline_gate_json,'$.preview_evidence_count')=21 AND json_extract(merged_outline_gate_json,'$.evidence_preservation_ratio')=1 AND json_array_length(json_extract(merged_outline_gate_json,'$.missing_evidence_ids'))=0").get().count,1,"the merged outline must preserve the complete 21-ID evidence union without silent loss");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM content_consolidation_blueprints WHERE json_extract(merged_title_gate_json,'$.state')='ready_for_editor_review' AND json_extract(merged_title_gate_json,'$.required_axis_count')=5 AND json_extract(merged_title_gate_json,'$.covered_axis_count')=5 AND json_extract(merged_title_gate_json,'$.source_evidence_count')=11 AND json_extract(merged_title_gate_json,'$.preview_evidence_count')=11").get().count,1,"the merged title preview must cover all five axes and preserve both title candidates' 11 evidence IDs");

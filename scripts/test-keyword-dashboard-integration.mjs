...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM competitor_page_terms").get().count,75325);
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  assert.equal(actual.sites[0].data_disposition.length,14);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_cooccurrence_projection").observed_count,16995);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_cooccurrence_projection").delta_count,0);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="wordpress_paragraph_text").state,"intentionally_not_retained");assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="keyword_serp").delta_count,10594);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="gsc_normalization").delta_count,3);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").delta_count,10);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").state,"partial_with_serp_fallback");assert.equal(actual.competitor_summary.failed_page_with_serp_snippet_count,10);assert.equal(actual.competitor_summary.serp_snippet_observation_count,300);assert.equal(actual.competitor_serp_snippet_evidence.filter((row)=>actual.competitor_pages.find((page)=>page.page_id===row.page_id)?.status!=="ok").length,11);assert.ok(actual.competitor_serp_snippet_evidence.every((row)=>row.evidence_digest.length===64&&row.source_snapshot_digest.length===64));assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="paa_answers").observed_count,0);assert.ok(actual.sites[0].data_disposition.every((row)=>row.evidence_digest.length===64));
  ...
  assert.deepEqual({...actual.sites[0].content_generation_quality_oracle,issue_counts:undefined,oracle_digest:undefined},{policy:"evidence-bound-review.v3",candidate_count:747,title_count:179,heading_count:568,ready_count:726,needs_review_count:18,blocked_count:3,evidence_resolved_count:747,competitor_copy_risk_count:0,existing_collision_count:1,issue_counts:undefined,oracle_digest:undefined});assert.equal(actual.sites[0].content_generation_quality_oracle.oracle_digest.length,64);assert.equal(actual.sites[0].content_generation_quality_oracle.issue_counts.cross_group_candidate_duplicate,2);assert.ok(actual.content_generation_candidates.every((row)=>row.review.oracle.evidence_reference_resolved&&row.review.review_policy==="evidence-bound-review.v3"));assert.equal(actual.content_generation_candidates.filter((row)=>row.generation.variant_key==="competitor_heading_fallback").length,32);assert.equal(actual.content_generation_candidates.filter((row)=>row.coverage.topic_assignment_policy==="source_task_fallback").length,16);assert.equal(actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="brief_ready").length,63);assert.deepEqual(actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="blocked").map((row)=>row.group_id),[]);

...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  assert.equal(actual.sites[0].data_disposition.length,10);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="keyword_serp").delta_count,10594);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="gsc_normalization").delta_count,3);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").delta_count,10);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="paa_answers").observed_count,0);assert.ok(actual.sites[0].data_disposition.every((row)=>row.evidence_digest.length===64));

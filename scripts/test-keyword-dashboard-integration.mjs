...
const mcpTools=await fetch(`http://127.0.0.1:${port}/mcp`,{method:"POST",headers:{"content-type":"application/json","mcp-protocol-version":"2025-06-18"},body:JSON.stringify({jsonrpc:"2.0",id:2,method:"tools/list"})});assert.equal(mcpTools.status,200);const mcpToolList=(await mcpTools.json()).result.tools;assert.equal(mcpToolList.length,13);assert.deepEqual(new Set(mcpToolList.map((tool)=>tool.name)),new Set(["search_keywords","search_demands","list_question_candidates","get_content_brief","get_acquisition_status","analyze_titles","analyze_headings","list_outlines","analyze_competitor_domains","inspect_serp_results","track_rank_history","get_market_data","search_serp_feature_items"]));

...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  assert.deepEqual({...actual.sites[0].content_generation_quality_oracle,issue_counts:undefined,oracle_digest:undefined},{policy:"evidence-bound-review.v3",candidate_count:699,title_count:143,heading_count:556,ready_count:678,needs_review_count:18,blocked_count:3,evidence_resolved_count:699,competitor_copy_risk_count:0,existing_collision_count:1,issue_counts:undefined,oracle_digest:undefined});assert.equal(actual.sites[0].content_generation_quality_oracle.oracle_digest.length,64);assert.equal(actual.sites[0].content_generation_quality_oracle.issue_counts.cross_group_candidate_duplicate,2);assert.ok(actual.content_generation_candidates.every((row)=>row.review.oracle.evidence_reference_resolved&&row.review.review_policy==="evidence-bound-review.v3"));

...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  assert.equal(actual.sites[0].data_disposition.length,11);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="keyword_serp").delta_count,10594);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="gsc_normalization").delta_count,3);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").delta_count,10);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="paa_answers").observed_count,0);assert.ok(actual.sites[0].data_disposition.every((row)=>row.evidence_digest.length===64));
...
assert.match(app,/renderSurfaceInventory/);assert.match(app,/public_surface_inventory/);assert.match(app,/surface_only_unconnected/);assert.match(html,/sitemap公開URL母集団/);assert.match(html,/固定ページ・HTML sitemap/);

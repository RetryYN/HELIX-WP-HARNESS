...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  assert.deepEqual({...actual.sites[0].content_generation_quality_oracle,issue_counts:undefined,oracle_digest:undefined},{policy:"evidence-bound-review.v3",candidate_count:750,title_count:179,heading_count:571,ready_count:727,needs_review_count:18,blocked_count:5,evidence_resolved_count:750,competitor_copy_risk_count:3,existing_collision_count:1,issue_counts:undefined,oracle_digest:undefined});assert.equal(actual.sites[0].content_generation_quality_oracle.oracle_digest.length,64);assert.equal(actual.sites[0].content_generation_quality_oracle.issue_counts.cross_group_candidate_duplicate,2);assert.ok(actual.content_generation_candidates.every((row)=>row.review.oracle.evidence_reference_resolved&&row.review.review_policy==="evidence-bound-review.v3"));assert.equal(actual.content_generation_candidates.filter((row)=>row.generation.variant_key==="competitor_heading_fallback").length,14);assert.equal(actual.content_generation_candidates.filter((row)=>row.coverage.topic_assignment_policy==="source_task_fallback").length,10);assert.equal(actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="brief_ready").length,63);assert.deepEqual(actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="blocked").map((row)=>row.group_id),[]);
  assert.equal(actual.content_outlines.length,63);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.candidate_count,0),571);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.selected_count,0),503);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.h2_count,0),244);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.h3_count,0),259);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.unassigned_candidates.length,0),63);assert.ok(actual.content_outlines.every((row)=>row.policy==="evidence-outline-selection.v1"&&!row.auto_approval));assert.ok(actual.content_outlines.flatMap((row)=>row.sections.flatMap((section)=>section.children)).every((child)=>child.parent_relation.parent_candidate_id&&child.parent_relation.policy==="shared-evidence-then-character-trigram.v1"&&(child.parent_relation.shared_evidence_count>0||child.parent_relation.lexical_jaccard>0||child.parent_relation.substring_match)));

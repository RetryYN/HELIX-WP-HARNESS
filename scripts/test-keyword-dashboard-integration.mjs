...
const fixturePath=fromRepo("docs/prototypes/wp-ops-dashboard/data.json");
const fixture=JSON.parse(readFileSync(fixturePath,"utf8"));
const SITE_ID=fixture.sites.find((site)=>site.site_id===fixture.groups[0].site_id).site_id;
const OTHER_SITE_ID=fixture.sites.find((site)=>site.site_id!==SITE_ID).site_id;
const pocInputPath=fromRepo(process.env.WP_KEYWORD_POC_RESULT??"artifacts/poc/keyword-workbook-100-live/result.json");
const pocInput=JSON.parse(readFileSync(pocInputPath,"utf8"));
const keywordWorkbookPath=fromRepo(process.env.WP_KEYWORD_WORKBOOK??"../poc-wp/data/IT就活大学キーワードマップ.xlsx");
const workbookRows=existsSync(keywordWorkbookPath)?readXlsxKeywordWorkbook(keywordWorkbookPath):null;
const hasKeywordWorkbook=workbookRows!==null;
const inputRows=workbookRows??pocInput.tasks;
const expectedKeywordCount=inputRows.length;
const expectedSheetCount=new Set(inputRows.map((row)=>row.source_sheet)).size;
const expectedUnacquiredKeywordCount=expectedKeywordCount-100;
const competitorEvidencePath=fromRepo(process.env.WP_COMPETITOR_EVIDENCE??".helix/evidence/competitor-content/manifest.json");
const hasCompetitorEvidence=existsSync(competitorEvidencePath);
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM imported_keywords WHERE site_id = ?").get(SITE_ID).count,expectedKeywordCount);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_hierarchy").get().count,expectedKeywordCount,"every imported keyword must have a hierarchy row");
assert.equal(pocDb.prepare("SELECT COUNT(DISTINCT source_sheet) AS count FROM imported_keywords").get().count,expectedSheetCount);
const contextItCount=pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_hierarchy WHERE context_scope_id='context:it'").get().count;
const contextGeneralCount=pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_hierarchy WHERE context_scope_id='context:general'").get().count;
assert.equal(contextItCount+contextGeneralCount,expectedKeywordCount,"context hierarchy rows must reconcile to the imported input rows");
if(hasKeywordWorkbook){
  assert.equal(contextItCount,2048);
  assert.equal(contextGeneralCount,8646);
  ...
  assert.ok(contextItCount>0&&contextGeneralCount>0,"fallback input must retain both hierarchy contexts");
  assert.ok(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_hierarchy WHERE term_count>0").get().count>0,"fallback hierarchy must retain term counts");
  assert.ok(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_hierarchy WHERE parent_source_keyword_id IS NOT NULL").get().count>0,"fallback hierarchy must retain parent relationships");
}
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM imported_keywords WHERE site_id = ? AND processing_state = '施策KW群割当済み'").get(SITE_ID).count, 100);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM imported_keywords WHERE site_id = ? AND processing_state = 'SERP未取得'").get(SITE_ID).count,expectedUnacquiredKeywordCount,"unacquired workbook rows must remain explicit rather than disappearing");
const variantClusterCount=pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_variant_clusters WHERE site_id=?").get(SITE_ID).count;
const associationCount=pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_term_associations WHERE site_id=?").get(SITE_ID).count;
if(hasKeywordWorkbook){assert.equal(variantClusterCount,921);assert.equal(associationCount,1636)}else{assert.ok(variantClusterCount>0,"fallback input must produce variant clusters");assert.ok(associationCount>0,"fallback input must produce term associations")};assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_term_associations WHERE term='.' OR (term GLOB '[0-9]*' AND term NOT GLOB '*[^0-9]*')").get().count,0,"punctuation/numeric corpus noise must not enter the association ranking");
const relatedKeywordProposalCount=pocDb.prepare("SELECT COUNT(*) AS count FROM related_keyword_proposals").get().count;
const relatedKeywordProposalGroupCount=pocDb.prepare("SELECT COUNT(DISTINCT group_id) AS count FROM related_keyword_proposals").get().count;
if(hasKeywordWorkbook){assert.equal(relatedKeywordProposalCount,792);assert.equal(relatedKeywordProposalGroupCount,60)}else{assert.equal(relatedKeywordProposalCount,0,"fallback rows are all acquired, so no related-keyword inventory remains");assert.equal(relatedKeywordProposalGroupCount,0)}assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM related_keyword_proposals WHERE status!='proposed' OR derivation_policy!='inventory-related-keyword.v2'").get().count,0);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_groups WHERE site_id = ?").get(SITE_ID).count, 64);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_groups WHERE site_id = ? AND resolution_state = 'resolved' AND main_keyword IS NOT NULL").get(SITE_ID).count, 63, "every real group currently resolves to an actual main keyword");
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_groups WHERE site_id = ? AND action_state = '未施策'").get(SITE_ID).count,64,"article matching must not infer workflow state");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM keyword_groups WHERE site_id = ? AND action_state = '公開中'").get(SITE_ID).count,0,"published state requires separate WP lifecycle evidence");
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM dfs_tasks WHERE group_id IN (SELECT group_id FROM keyword_groups WHERE site_id = ?)").get(SITE_ID).count, 100);
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM articles WHERE site_id = ? AND gsc_status = 'ok'").get(SITE_ID).count,59);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_structures WHERE site_id=?").get(SITE_ID).count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_observed_links WHERE site_id=?").get(SITE_ID).count,2539);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_observed_links WHERE site_id=? AND is_internal=1").get(SITE_ID).count,2061);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_images WHERE site_id=?").get(SITE_ID).count,1222);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_sections WHERE site_id=?").get(SITE_ID).count,1499);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_content_paragraphs WHERE site_id=?").get(SITE_ID).count,8050);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_page_seo_metadata WHERE site_id=?").get(SITE_ID).count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_page_seo_metadata WHERE error IS NULL AND http_status=200 AND length(evidence_digest)=64").get().count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM wp_page_seo_metadata WHERE canonical_url IS NULL OR description IS NULL OR title='' OR head_digest IS NULL").get().count,0,"all public pages must retain core SEO head evidence without retaining HTML");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM site_public_urls WHERE site_id=?").get(SITE_ID).count,70);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM site_public_urls WHERE surface_type='post'").get().count,59);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM site_public_urls WHERE surface_type='page'").get().count,9);assert.equal(pocDb.prepare("SELECT url_count FROM site_surface_runs WHERE site_id=?").get(SITE_ID).url_count,70);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM public_page_surface_observations WHERE site_id=? AND http_status=200 AND error IS NULL").get(SITE_ID).count,70);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM public_page_surface_links WHERE site_id=?").get(SITE_ID).count,12586);assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM public_page_surface_links WHERE site_id=? AND is_internal=1").get(SITE_ID).count,11707);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM competitor_page_terms").get().count,hasCompetitorEvidence?207871:0);
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM articles WHERE site_id=? AND modified_at IS NOT NULL AND length(headings_digest)=64").get(SITE_ID).count,59,"WP modified time and heading evidence digest must survive projection");
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE site_id = ?").get(SITE_ID).count,681);
...
assert.equal(pocDb.prepare("SELECT COUNT(*) AS count FROM gsc_query_results WHERE normalized_query = ''").get().count,0);
{
  ...
  if(hasKeywordWorkbook){assert.equal(actual.sites[0].lexical_index.variant_clusters.length,921);assert.equal(actual.sites[0].lexical_index.associations.length,1636)}else{assert.ok(actual.sites[0].lexical_index.variant_clusters.length>0,"fallback input must project variant clusters");assert.ok(actual.sites[0].lexical_index.associations.length>0,"fallback input must project term associations")};assert.ok(actual.sites[0].lexical_index.associations.every((item)=>item.evidence_source_keyword_ids.length>0&&item.evidence_digest.length===64));
  ...
  assert.equal(actual.sites[0].portfolio_metrics.keyword_count,expectedKeywordCount);assert.equal(actual.sites[0].portfolio_metrics.serp_acquired_keyword_count,100);assert.equal(actual.sites[0].portfolio_metrics.serp_unacquired_keyword_count,expectedUnacquiredKeywordCount);assert.equal(actual.sites[0].portfolio_metrics.task_count,100);assert.equal(actual.sites[0].portfolio_metrics.gsc_query_count,678);assert.equal(actual.sites[0].portfolio_metrics.gsc_query_row_count,681,"normalized query count and retained source rows must both remain visible");assert.equal(actual.sites[0].portfolio_metrics.question_candidate_count,actual.sites[0].ai_question_candidates.length);
  assert.equal(actual.sites[0].data_disposition.length,15);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_cooccurrence_projection").observed_count,hasCompetitorEvidence?46870:0);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_cooccurrence_projection").delta_count,0);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="wordpress_paragraph_text").state,"intentionally_not_retained");assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="keyword_serp").delta_count,expectedUnacquiredKeywordCount);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="gsc_normalization").delta_count,3);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").delta_count,hasCompetitorEvidence?29:0);assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="competitor_content").state,"partial_with_serp_fallback");assert.equal(actual.competitor_summary.failed_page_with_serp_snippet_count,hasCompetitorEvidence?29:0);assert.equal(actual.competitor_summary.serp_snippet_observation_count,hasCompetitorEvidence?925:0);assert.equal(actual.competitor_serp_snippet_evidence.filter((row)=>actual.competitor_pages.find((page)=>page.page_id===row.page_id)?.status!=="ok").length,hasCompetitorEvidence?36:0);assert.ok(actual.competitor_serp_snippet_evidence.every((row)=>row.evidence_digest.length===64&&row.source_snapshot_digest.length===64));assert.equal(actual.sites[0].data_disposition.find((row)=>row.dataset==="paa_answers").observed_count,0);assert.ok(actual.sites[0].data_disposition.every((row)=>row.evidence_digest.length===64));
  ...
  const expectedQualityOracle=hasCompetitorEvidence?{policy:"evidence-bound-review.v3",candidate_count:750,title_count:179,heading_count:571,ready_count:727,needs_review_count:18,blocked_count:5,evidence_resolved_count:750,competitor_copy_risk_count:3,existing_collision_count:1,issue_counts:undefined,oracle_digest:undefined}:{policy:"evidence-bound-review.v3",candidate_count:546,title_count:126,heading_count:420,ready_count:509,needs_review_count:18,blocked_count:19,evidence_resolved_count:546,competitor_copy_risk_count:0,existing_collision_count:1,issue_counts:undefined,oracle_digest:undefined};assert.deepEqual({...actual.sites[0].content_generation_quality_oracle,issue_counts:undefined,oracle_digest:undefined},expectedQualityOracle);assert.equal(actual.sites[0].content_generation_quality_oracle.oracle_digest.length,64);assert.equal(actual.sites[0].content_generation_quality_oracle.issue_counts.cross_group_candidate_duplicate,hasCompetitorEvidence?2:18);assert.ok(actual.content_generation_candidates.every((row)=>row.review.oracle.evidence_reference_resolved&&row.review.review_policy==="evidence-bound-review.v3"));assert.equal(actual.content_generation_candidates.filter((row)=>row.generation.variant_key==="competitor_heading_fallback").length,hasCompetitorEvidence?14:0);assert.equal(actual.content_generation_candidates.filter((row)=>row.coverage.topic_assignment_policy==="source_task_fallback").length,hasCompetitorEvidence?10:56);assert.equal(actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="brief_ready").length,hasCompetitorEvidence?63:44);const blockedStructureCandidates=actual.content_structure_candidates.filter((row)=>row.draft_package.generation_state==="blocked");assert.equal(blockedStructureCandidates.length,hasCompetitorEvidence?0:19);if(hasCompetitorEvidence)assert.deepEqual(blockedStructureCandidates.map((row)=>row.group_id),[]);else assert.ok(blockedStructureCandidates.every((row)=>row.draft_package.generation_state==="blocked"));
  assert.equal(actual.content_outlines.length,63);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.candidate_count,0),hasCompetitorEvidence?571:420);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.selected_count,0),hasCompetitorEvidence?503:229);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.h2_count,0),hasCompetitorEvidence?244:60);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.h3_count,0),hasCompetitorEvidence?259:169);assert.equal(actual.content_outlines.reduce((sum,row)=>sum+row.unassigned_candidates.length,0),hasCompetitorEvidence?63:172);assert.ok(actual.content_outlines.every((row)=>row.policy==="evidence-outline-selection.v1"&&!row.auto_approval));assert.ok(actual.content_outlines.flatMap((row)=>row.sections.flatMap((section)=>section.children)).every((child)=>child.parent_relation.parent_candidate_id&&child.parent_relation.policy==="shared-evidence-then-character-trigram.v1"&&(child.parent_relation.shared_evidence_count>0||child.parent_relation.lexical_jaccard>0||child.parent_relation.substring_match)));
  ...
  assert.equal(actual.groups.reduce((sum,group)=>sum+group.related_keyword_proposals.length,0),relatedKeywordProposalCount);assert.ok(actual.groups.flatMap((group)=>group.related_keyword_proposals).every((item)=>item.evidence.representative_source_keyword_id&&item.evidence_digest.length===64));
  ...
  assert.equal(actual.serp_action_signals.length,25);assert.ok(actual.serp_action_signals.some((row)=>row.signal_types.includes("visual")&&row.recommended_formats.includes("original_images")));assert.ok(actual.serp_action_signals.some((row)=>row.signal_types.includes("commercial")&&row.rated_result_count===2));assert.ok(actual.serp_action_signals.some((row)=>row.signal_types.includes("spelling")&&row.corrected_keyword==="就活ネクタイ"));assert.equal(actual.serp_action_signals.reduce((sum,row)=>sum+row.video_result_count,0),19);assert.equal(actual.serp_action_signals.filter((row)=>row.signal_types.includes("video")).length,20);assert.ok(actual.serp_action_signals.every((row)=>row.status==="proposed"&&row.evidence.length>0));
  ...
  assert.equal(actual.primary_query_ranking[SITE_ID].impression_p95,41,"ranking threshold must be derived from normalized actual GSC distribution");
  const confirmed=new Map(actual.groups.filter((group)=>group.site_id===SITE_ID&&group.article_match?.state==="確定").map((group)=>[group.main_keyword,group.wp_article_id]));
...
// §5: a derived_parent_candidate group must persist as unresolved, stay visible, and never be matched to or assigned a WP article.
{
  ...
  const template=baseFixture.groups.find((group)=>group.site_id===SITE_ID);
  ...
  assert.throws(()=>db.prepare("INSERT INTO keyword_groups (group_id,site_id,resolution_state,main_keyword,derived_parent_candidate,main_origin,category,category_path_json,source_order_file,source_order_sheet,source_order_row,source_location,search_volume_json,search_volume_source,confidence,overlap_shared,overlap_depth,overlap_ratio,action_state,wp_article_id,cost) VALUES ('bad',?,'resolved',NULL,NULL,'x','c','[]',0,0,0,'l','null','s','single',0,5,0,'未施策',NULL,0)").run(SITE_ID),/CHECK/,"a resolved group without a main keyword must be rejected by the schema");
  db.close();
}
...
assert.equal(first.sites.length, 2);
  assert.equal(first.sites.find((site)=>site.site_id===OTHER_SITE_ID).portfolio_metrics.group_count,1);assert.equal(first.sites.find((site)=>site.site_id===SITE_ID).portfolio_metrics.group_count,1,"portfolio metrics must remain site scoped rather than use the two-site total");
assert.equal(first.groups.length, 2);
const solo = first.groups.filter((group) => group.site_id === OTHER_SITE_ID);

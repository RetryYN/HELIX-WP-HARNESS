const specs={
  keyword_metrics:{endpoint:"/keywords_data/google_ads/search_volume/live",limit:1000,cost_model:"per_request",request_cost_usd:.09,source:"DataForSEO Google Ads Search Volume live"},
  keyword_difficulty:{endpoint:"/dataforseo_labs/google/bulk_keyword_difficulty/live",limit:1000,cost_model:"task_plus_returned_item",request_cost_usd:.012,item_cost_usd:.00012,source:"DataForSEO Labs Bulk Keyword Difficulty live"},
  ranked_keywords:{endpoint:"/dataforseo_labs/google/ranked_keywords/live",limit:1000,cost_model:"task_plus_returned_item",request_cost_usd:.012,item_cost_usd:.00012,source:"DataForSEO Labs Ranked Keywords live"},
};
...
export function buildDfsEnrichmentPlan({keywords,target,locationCode=2392,languageCode="ja"}){
  const uniqueKeywords=[...new Set(keywords.map((value)=>String(value).trim()).filter(Boolean))];
  if(!uniqueKeywords.length)throw new Error("at least one keyword is required");if(!target)throw new Error("target domain is required");
  ...
  return{schema_version:"dfs-enrichment-plan.v1",provider:"DataForSEO",location_code:locationCode,language_code:languageCode,target,keyword_count:uniqueKeywords.length,pricing_basis:{captured_on:"2026-08-26",specs},jobs,estimated_max_usd:round(jobs.reduce((sum,job)=>sum+job.estimated_max_usd,0)),execution_guard:"live requires explicit WP_DFS_ENRICHMENT_LIVE=1, selected jobs, account price snapshot and a sufficient cost ceiling"};
}

export function selectDfsEnrichmentJobs(plan,kinds){const wanted=new Set(kinds);const jobs=plan.jobs.filter((job)=>wanted.has(job.kind));if(!jobs.length)throw new Error("no acquisition jobs selected");return{...plan,jobs,estimated_max_usd:round(jobs.reduce((sum,job)=>sum+job.estimated_max_usd,0))}}

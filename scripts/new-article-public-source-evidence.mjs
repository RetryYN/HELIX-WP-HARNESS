...
export function buildNewArticlePublicSourceEvidence(manifest,observations){
  ...
  const rows=(observations.rows??[]).map((observation)=>{
    ...
    if(!observation.executed_query)throw new Error(`executed query missing: ${observation.claim_id}`);
    if(!["planned_query_executed","adapted_for_authoritative_entity_source"].includes(observation.query_strategy_state))throw new Error(`invalid query strategy: ${observation.claim_id}`);
    if(observation.query_strategy_state==="planned_query_executed"&&observation.executed_query!==observation.query)throw new Error(`planned query execution mismatch: ${observation.claim_id}`);
    if(observation.query_strategy_state==="adapted_for_authoritative_entity_source"&&!observation.adaptation_reason)throw new Error(`query adaptation reason missing: ${observation.claim_id}`);
  ...
  const summary={checked_claim_count:rows.length,qualifying_candidate_count:rows.filter((row)=>row.result_state==="qualifying_candidate_observed").length,no_qualifying_result_count:rows.filter((row)=>row.result_state==="no_qualifying_result").length,planned_query_executed_count:rows.filter((row)=>row.query_strategy_state==="planned_query_executed").length,adapted_query_count:rows.filter((row)=>row.query_strategy_state==="adapted_for_authoritative_entity_source").length,directly_supported_count:rows.filter((row)=>row.direct_support_state==="directly_supported").length,contextual_only_count:rows.filter((row)=>row.direct_support_state==="contextual_only").length,not_supported_count:rows.filter((row)=>row.direct_support_state==="not_supported").length,requirement_satisfied_count:rows.filter((row)=>row.requirement_satisfied).length,acquisition_cost_usd:0,external_public_review_executed_count:rows.length,auto_approval_count:0,auto_publication_count:0};

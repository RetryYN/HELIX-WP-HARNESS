...
const keywords=Array.from({length:100},(_,index)=>`kw ${index}`),plan=buildDfsEnrichmentPlan({siteId:"it-shukatu.com",keywords,target:"it-shukatu.com"});
...
assert.equal(plan.site_id,"it-shukatu.com");assert.throws(()=>buildDfsEnrichmentPlan({keywords,target:"x.example"}),/site identity/);assert.throws(()=>buildDfsEnrichmentPlan({siteId:"x.example",keywords:["x ".repeat(11)],target:"x.example"}),/provider limit/);assert.throws(()=>selectDfsEnrichmentJobs(plan,["unknown"]),/no acquisition jobs/);

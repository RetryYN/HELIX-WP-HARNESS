...
try {
  ...
  assert.ok(details.body.data[0].rakko_semantics.page_details.length>0);assert.equal(details.body.data[0].rakko_semantics.occurrence_page_count,details.body.data[0].total_count);assert.equal(details.body.data[0].rakko_semantics.site_count_total,new Set(details.body.data[0].rakko_semantics.page_details.map((row)=>row.domain)).size);assert.ok(details.body.data[0].rakko_semantics.page_details.every((row)=>row.page_count===1&&row.count>=0&&typeof row.in_heading==="boolean"));

...
try {
  ...
  assert.equal(summary.evidence_safe_revision_proposal_count,8);
  assert.equal(summary.evidence_safe_manual_revision_review_required_count,0);
  assert(summary.evidence_safe_revision_group_count>0);
  assert.equal(summary.evidence_safe_revision_decision_blocked_count,summary.evidence_safe_revision_group_count);
  assert.equal(summary.evidence_safe_manual_revision_blocked_count,summary.evidence_safe_revision_group_count);
  ...
  assert(api.body.data.filter((row)=>row.evidence_safe_revision.proposal_count).every((row)=>row.blocker_codes.includes("evidence_safe_revision_decision")&&row.blocker_codes.includes("evidence_safe_manual_revision")&&row.evidence_safe_revision.proposal_set_digest===site.evidence_safe_draft_revisions.proposal_set_digest&&!row.evidence_safe_revision.artifact_applied));

...
const consumerRules=[
  ...
  [/^people_also_ask\.items\[\]\.(?:type|xpath)$/u,"scripts/paa-answer-evidence.mjs","question.type","PAA question answer-state lineage"],
  [/^people_also_ask\.items\[\]\.expanded_element\[\]\.(?:type|asynchronous_ai_overview|url|domain|title|description|featured_title|timestamp)$/u,"scripts/paa-answer-evidence.mjs","item.url","resolved/pending PAA answer evidence"],
  [/^people_also_ask\.items\[\]\.expanded_element\[\]\.table\.(?:table_header\[\]|table_content\[\]\[\])$/u,"scripts/paa-answer-evidence.mjs","table_content","PAA answer table preservation"],
...
export function auditSerpDataCoverage(rawRoot=defaultRoot){
  ...
  for(const name of files){
    ...
    for(const item of result.items??[]){
      ...
      if(item.type==="people_also_ask")for(const question of item.items??[]){paaQuestions+=1;if(present(question.title))bump("people_also_ask.items.title");if(present(question.seed_question))bump("people_also_ask.items.seed_question");for(const expanded of question.expanded_element??[]){if([expanded.url,expanded.description,expanded.featured_title,expanded.table].some(present))paaAnswers+=1;paaReferences+=(expanded.references??[]).length}}

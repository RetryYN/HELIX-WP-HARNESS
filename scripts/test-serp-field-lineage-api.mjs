...
const dashboard={sites:[],serp_field_lineage:JSON.parse(readFileSync("docs/prototypes/wp-ops-dashboard/serp-data-coverage-audit.json","utf8"))};
{
  ...
  const evidenceUrl=new URL("http://localhost/api/v1/serp-field-lineage?q=organic.timestamp&decision_state=evidence_only"),evidence=routeResearchApi(evidenceUrl.pathname,evidenceUrl,dashboard);assert.equal(evidence.body.meta.total,1);assert.equal(evidence.body.data[0].consumer,null);

...
const dashboard={sites:[],serp_field_lineage:JSON.parse(readFileSync("docs/prototypes/wp-ops-dashboard/serp-data-coverage-audit.json","utf8"))};
{
  ...
  assert.equal(response.status,200);assert.equal(response.body.summary.field_count,179);assert.equal(response.body.summary.projected_field_count,179);assert.equal(response.body.summary.raw_only_field_count,0);assert.equal(response.body.summary.consumer_verified_field_count,109);assert.equal(response.body.summary.consumer_missing_field_count,0);assert.equal(response.body.meta.total,109);assert.ok(response.body.data.every((row)=>row.projection_state==="projected"&&row.decision_state==="decision_connected"&&row.consumer?.verification_state==="verified_source_reference"));assert.equal(response.body.provenance.external_acquisition_triggered,false);
  ...
  const mcp=handleMcpMessage({jsonrpc:"2.0",id:1,method:"tools/call",params:{name:"audit_serp_field_lineage",arguments:{decision_state:"decision_connected",limit:100}}},dashboard);assert.equal(mcp.result.structuredContent.meta.total,109);assert.ok(mcp.result.structuredContent.data.every((row)=>row.consumer?.verification_state==="verified_source_reference"));
  console.log("SERP field lineage API/MCP: OK (179 primitive paths, 109 source-verified consumers, raw-only zero)");
}

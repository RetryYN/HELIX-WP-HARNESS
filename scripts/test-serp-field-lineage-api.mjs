...
const dashboard={sites:[],serp_field_lineage:JSON.parse(readFileSync("docs/prototypes/wp-ops-dashboard/serp-data-coverage-audit.json","utf8"))};
{
  ...
  assert.equal(response.status,200);assert.equal(response.body.summary.field_count,179);assert.equal(response.body.summary.projected_field_count,179);assert.equal(response.body.summary.raw_only_field_count,0);assert.equal(response.body.summary.consumer_verified_field_count,179);assert.equal(response.body.summary.consumer_missing_field_count,0);assert.equal(response.body.summary.decision_connected_field_count,85);assert.equal(response.body.summary.evidence_only_field_count,94);assert.equal(response.body.meta.total,85);assert.ok(response.body.data.every((row)=>row.projection_state==="projected"&&row.decision_state==="decision_connected"&&row.consumer?.verification_state==="verified_source_reference"));assert.equal(response.body.provenance.external_acquisition_triggered,false);

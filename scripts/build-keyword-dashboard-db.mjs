...
const gscEvidencePath=fromRepo(process.env.WP_GSC_EVIDENCE??"artifacts/poc/evidence-fixtures/gsc-page-query-28d/manifest.json");
...
const counts = Object.fromEntries(["sites", "imported_keywords", "keyword_groups", "dfs_tasks", "serp_demand_occurrences", "serp_organic_results", "serp_page_keyword_edges", "simultaneous_keyword_relations", "serp_page_coverage", "serp_domain_coverage", "serp_ai_overviews", "content_topic_proposals", "content_topic_coverage", "content_structure_candidates", "content_generation_candidates", "competitor_pages", "competitor_headings", "competitor_terms", "competitor_task_terms", "gate_runs", "articles", "gsc_query_results", "article_links"].map((table) => [table, Number(db.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get().count)]));
db.close();

...
const remediationRoot=fromRepo(process.env.WP_ACQUISITION_REMEDIATION_ROOT??".helix/evidence/acquisition-remediation");
const lifetimeLedgerRoot=fromRepo(process.env.WP_ACQUISITION_LIFETIME_LEDGER_ROOT??remediationRoot);
const legacyCommittedEntries=poc.tasks.map((row)=>({source_id:row.task_id,cost:row.cost,observed_at:row.observed_at,source_digest:row.response_digest})),lifetimeAudit=auditAcquisitionLifetimeBudget(lifetimeLedgerRoot,{lifetimeCapUsd:5,plannedMaximumCostUsd:0,legacyCommittedEntries}),acquisitionLifetimeState={scope:lifetimeAudit.scope,currency:lifetimeAudit.currency,lifetime_cap_usd:lifetimeAudit.lifetime_cap_usd,legacy_committed_cost_usd:lifetimeAudit.legacy_committed_cost_usd,run_committed_cost_usd:lifetimeAudit.run_committed_cost_usd,committed_cost_usd:lifetimeAudit.committed_cost_usd,remaining_budget_usd:lifetimeAudit.remaining_before_run_usd,legacy_committed_entry_count:lifetimeAudit.legacy_committed_entry_count,committed_run_count:lifetimeAudit.committed_run_count,legacy_committed_entries:lifetimeAudit.legacy_committed_entries,budget_digest:lifetimeAudit.budget_digest};
const lifetimeCostEvidence=loadAcquisitionRemediationEvidence(lifetimeLedgerRoot),acquisitionCostHistoryEntries=lifetimeCostEvidence?[...lifetimeCostEvidence.results.map((row)=>({provider:"data-provider-b",cost_unit:"USD",source_kind:"acquisition_result_cost",source_id:row.acquisition_task_id,endpoint:"/retained-acquisition-result",observed_at:row.observed_at,cost:Number(row.cost),source_digest:row.response_digest})),...Number((lifetimeCostEvidence.manifest.provider_reported_post_cost_usd-lifetimeCostEvidence.manifest.actual_completed_cost_usd).toFixed(8))>0?[{provider:"data-provider-b",cost_unit:"USD",source_kind:"acquisition_run_non_result_cost",source_id:"site-a.example:acquisition-run-cost-adjustment",endpoint:"/acquisition/run-adjustment",observed_at:lifetimeCostEvidence.manifest.updated_at,cost:Number((lifetimeCostEvidence.manifest.provider_reported_post_cost_usd-lifetimeCostEvidence.manifest.actual_completed_cost_usd).toFixed(8)),source_digest:lifetimeAudit.budget_digest}]:[]]:[];
const activeRemediation=loadAcquisitionRemediationEvidence(remediationRoot),remediationBySource=new Map((activeRemediation?.results??[]).map((row)=>[row.source_task_id,row]));
const acquisitionRemediationExecution=activeRemediation?{state:activeRemediation.manifest.state,created_at:activeRemediation.manifest.created_at,updated_at:activeRemediation.manifest.updated_at,approved_budget:{currency:"USD",amount:activeRemediation.manifest.approval.hard_cap},planned_maximum_cost_usd:activeRemediation.manifest.approval.plan_maximum,reported_cost_usd:activeRemediation.manifest.provider_reported_post_cost_usd,completed_result_cost_usd:activeRemediation.manifest.actual_completed_cost_usd,accepted_task_count:activeRemediation.manifest.accepted_task_count,rejected_task_count:activeRemediation.manifest.rejected_task_count,completed_task_count:activeRemediation.manifest.completed_task_count,result_index_digest:activeRemediation.manifest.result_index_digest,auto_resubmission:false}:null;
const keywordWorkbookPath=fromRepo(process.env.WP_KEYWORD_WORKBOOK??"../poc-wp/data/サイトAキーワードマップ.xlsx"),workbookRows=existsSync(keywordWorkbookPath)?readXlsxKeywordWorkbook(keywordWorkbookPath):null;
const importedKeywords=(workbookRows??poc.tasks).map((row)=>({source_keyword_id:row.source_keyword_id??`site-a.example:${row.source_sheet}:${row.source_row}`,site_id:"site-a.example",source_sheet:row.source_sheet,source_row:row.source_row,raw_keyword:row.raw_keyword??row.keyword,search_volume:row.search_volume,cpc:row.cpc,competition:row.competition}));
...
const processedGroups=poc.article_keyword_groups.map((group,index)=>{
  ...
  return {
    id:`it-shukatu-serp-${String(index+1).padStart(3,"0")}`,site_id:"site-a.example",resolution_state:resolved?"resolved":"unresolved",main_keyword:resolved?group.main_keyword:null,derived_parent_candidate:resolved?null:group.derived_parent_candidate??null,
    main_origin:resolved?mainBasis:"derived_parent_candidate（修飾語KWのみ・実在親KWなし・main未確定）",
    source_order:{file:0,sheet:0,row:Math.min(...rows.map((row)=>row.source_row))},source_location:`DB取込 / IT就活 / ${(main??rows[0]).source_row}行`,search_volume:resolved?group.main_search_volume:null,search_volume_source:"取込DB（DataProviderB検索Vol）",
    ...
    strategy:{decision:resolved?(rows.length>1?"1記事に統合":"単独施策候補"):"親KW未確定（PO確定またはDPB取得待ち）",article_count:1,main_basis:resolved?mainBasis:"derived_parent_candidate（未昇格）",click_opportunity:"AIO出現クエリは施策評価で別管理"},
    ...
    ]},cost:rows.reduce((sum,row)=>sum+Number(row.cost??0)+Number(remediationBySource.get(row.task_id)?.cost??0),0),task_ids:rows.map((row)=>remediationBySource.get(row.task_id)?.acquisition_task_id??row.task_id),shared_urls:sharedUrls
  };
});
const fixture={...baseFixture,generated_at:poc.generated_at,sites:baseFixture.sites.filter((site)=>site.site_id==="site-a.example"),groups:processedGroups};
...
const dataProviderBEnrichmentEvidencePath=process.env.WP_DATA_PROVIDER_B_ENRICHMENT_EVIDENCE?fromRepo(process.env.WP_DATA_PROVIDER_B_ENRICHMENT_EVIDENCE):undefined;if(dataProviderBEnrichmentEvidencePath&&!existsSync(dataProviderBEnrichmentEvidencePath))throw new Error(`DPB enrichment evidence not found: ${dataProviderBEnrichmentEvidencePath}`);
const acquisitionPreviouslyAttemptedTaskIds=lifetimeCostEvidence?[...new Set([...lifetimeCostEvidence.results.flatMap((row)=>[row.acquisition_task_id,row.source_task_id]),...(lifetimeCostEvidence.manifest.rejections??[]).flatMap((row)=>[row.acquisition_task_id,row.source_task_id])].filter(Boolean))]:[];
const db = buildDashboardDb({ dbPath, fixturePath, artifactRoot: fromRepo("artifacts/poc"),additionalArtifactRoots:activeRemediation?[activeRemediation.runPath]:[],acquisitionRemediationExecution,acquisitionLifetimeState,acquisitionCostHistoryEntries,acquisitionPreviouslyAttemptedTaskIds,taskIdRemap:new Map([...remediationBySource].map(([source,row])=>[source,row.acquisition_task_id])), importedKeywords, serpAcquiredSourceKeywordIds:poc.tasks.map((row)=>row.source_keyword_id), gscEvidencePaths:availableGscEvidencePaths, headingEvidencePath:hasHeadingEvidence?headingEvidencePath:undefined,contentStructureEvidencePath:availableGscEvidencePaths.length&&existsSync(contentStructureEvidencePath)?contentStructureEvidencePath:undefined,siteSurfaceEvidencePath:availableGscEvidencePaths.length&&existsSync(siteSurfaceEvidencePath)?siteSurfaceEvidencePath:undefined,publicPageSurfaceEvidencePath:availableGscEvidencePaths.length&&existsSync(publicPageSurfaceEvidencePath)?publicPageSurfaceEvidencePath:undefined, competitorEvidencePath:existsSync(competitorEvidencePath)?competitorEvidencePath:undefined,dataProviderBEnrichmentEvidencePath });
const counts = Object.fromEntries(["sites", "imported_keywords", "keyword_groups", "public_lexical_sources", "public_synonym_pairs", "public_semantic_sources", "public_semantic_senses", "public_semantic_definitions", "public_semantic_relations", "data_provider_b_enrichment_runs", "keyword_market_metrics", "keyword_monthly_searches", "keyword_difficulty_enrichment", "domain_ranked_keywords", "raw_snapshot_inventory", "raw_snapshot_payloads", "data_provider_b_tasks", "serp_task_metadata", "serp_action_signals", "serp_feature_occurrences", "serp_feature_items", "serp_feature_item_links", "serp_intent_fingerprints", "serp_intent_pair_reviews", "keyword_boundary_reviews", "serp_depth_stability", "content_topology_reviews", "serp_organic_attributes", "serp_demand_occurrences", "serp_organic_results", "serp_page_keyword_edges", "simultaneous_keyword_relations", "serp_page_coverage", "serp_domain_coverage", "serp_ai_overviews", "aio_citation_references", "aio_citation_domains", "aio_content_elements", "content_topic_proposals", "content_topic_coverage", "content_structure_candidates", "content_generation_candidates", "competitor_pages", "competitor_headings", "competitor_page_terms", "competitor_terms", "competitor_task_terms", "gate_runs", "articles", "gsc_query_results", "article_links"].map((table) => [table, Number(db.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get().count)]));
counts.observed_hashtag_occurrences=Number(db.prepare("SELECT COUNT(*) AS count FROM observed_hashtag_occurrences").get().count);
counts.observed_hashtag_evidence=Number(db.prepare("SELECT COUNT(*) AS count FROM observed_hashtag_evidence").get().count);
counts.observed_tag_content_coverage=Number(db.prepare("SELECT COUNT(*) AS count FROM observed_tag_content_coverage").get().count);

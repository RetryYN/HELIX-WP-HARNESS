...
export function verifyRoutingExactPageReview(ranked, headings, serp, semantic, packets, review) {
  assert.equal(ranked.value.schema_version, 'routing-canonical-ranked-keyword-evidence.v1');
  assert.equal(headings.value.schema_version, 'routing-canonical-heading-evidence.v1');
  assert.equal(serp.value.schema_version, 'routing-query-serp-acquisition.v2');
  assert.equal(serp.value.status, 'complete', 'SERP acquisition must be terminal');
  assert.equal(semantic.value.schema_version, 'routing-query-serp-review-aggregate.v1');
  ...
  assert.equal(ranked.value.serp_digest, serp.digest, 'ranked SERP source mismatch');
  assert.equal(headings.value.serp_digest, serp.digest, 'heading SERP source mismatch');
  assert.equal(semantic.value.manifest_digest, serp.digest, 'semantic SERP source mismatch');
  const serpQueries = unique(serp.value.results, 'query_id', 'SERP query');
  assert.equal(serpQueries.size, serp.value.total_candidates, 'SERP query count mismatch');
  ...
  const queryReviews = unique(semantic.value.candidates.flatMap((candidate) => candidate.query_reviews), 'query_id', 'semantic query review');
  ...
  const results = [];
  const observedQueryOwners = new Map();
  for (const [jobId, source] of packetPages) {
    const rankedJob = rankedJobs.get(jobId); const row = reviewPages.get(jobId);
    assert.equal(source.target_url, rankedJob.target_url, `${jobId}: target URL mismatch`);
    ...
    const rankedObservations = unique(rankedJob.observations, 'query_id', `${jobId} ranked observation`);
    const sourceObservations = unique(source.observations, 'query_id', `${jobId} packet observation`);
    assert.deepEqual(new Set(sourceObservations.keys()), new Set(rankedObservations.keys()), `${jobId}: observation coverage mismatch`);
    for (const observation of sourceObservations.values()) {
      const rankedObservation = rankedObservations.get(observation.query_id);
      const serpQuery = serpQueries.get(observation.query_id);
      const semanticReview = queryReviews.get(observation.query_id);
      assert(serpQuery, `${jobId}: SERP query missing`);
      assert.equal(semanticReview?.decision, 'canonical_query', `${jobId}: observation is not an accepted canonical query`);
      assert(!observedQueryOwners.has(observation.query_id), `${jobId}: query belongs to multiple pages`);
      observedQueryOwners.set(observation.query_id, jobId);
      assert.equal(observation.article_cluster_id, rankedObservation.article_cluster_id, `${jobId}: observation cluster mismatch`);
      assert.equal(observation.serp_raw_digest, rankedObservation.serp_raw_digest, `${jobId}: ranked SERP observation mismatch`);
      assert.equal(observation.serp_raw_digest, serpQuery.raw_digest, `${jobId}: SERP raw evidence mismatch`);
      assert.equal(source.target_url, serpQuery.rank1_url, `${jobId}: SERP rank-one URL mismatch`);
      assert.equal(observation.canonical_query, serpQuery.query_to_test, `${jobId}: canonical query text mismatch`);
      assert.deepEqual(observation.semantic_review, semanticReview, `${jobId}: semantic review mismatch`);
    }
    const sourceKeywords = unique(source.acquired_keywords, 'keyword_digest', `${jobId} source keyword`);
    assert.deepEqual(new Set(sourceKeywords.keys()), new Set(rankedJob.observed_keyword_digests), `${jobId}: acquired keyword evidence mismatch`);
    for (const keyword of sourceKeywords.values()) {
      assert.equal(keyword.keyword_digest, sha(normalize(keyword.keyword)), `${jobId}: acquired keyword text mismatch`);
      assert.equal(keyword.target_url, source.target_url, `${jobId}: acquired keyword page mismatch`);
    }
    ...
    const headingObservations = unique(headingSource.observations, 'query_id', `${jobId} heading observation`);
    assert.deepEqual(new Set(headingObservations.keys()), new Set(sourceObservations.keys()), `${jobId}: heading observation coverage mismatch`);
    for (const observation of sourceObservations.values()) {
      const headingObservation = headingObservations.get(observation.query_id);
      assert.equal(headingObservation.article_cluster_id, observation.article_cluster_id, `${jobId}: heading observation cluster mismatch`);
      assert.equal(headingObservation.raw_digest, observation.serp_raw_digest, `${jobId}: heading SERP observation mismatch`);
    }
    ...
    assert.deepEqual(source.heading_evidence.headings, headingSource.headings, `${jobId}: heading manifest content mismatch`);
    const expectedHeadings = headingSource.headings.map((heading) => ({ ...heading, heading_digest: sha(heading.text) }));
  ...
  const acceptedQueryIds = new Set([...queryReviews.values()].filter((row) => row.decision === 'canonical_query').map((row) => row.query_id));
  assert.deepEqual(new Set(observedQueryOwners.keys()), acceptedQueryIds, 'accepted canonical query coverage mismatch');
  const acceptedQueryCount = acceptedQueryIds.size;
  const finalPassQueryIds = new Set(results.filter((row) => row.page_state === 'pass').flatMap((row) => packetPages.get(row.job_id).observations.map((observation) => observation.query_id)));
  const finalPassQueries = finalPassQueryIds.size;
  return { schema_version: 'routing-exact-page-review-verification.v1', pages: results, summary: { accepted_canonical_queries: acceptedQueryCount, exact_pages: results.length, complete_keyword_corpora: results.filter((row) => rankedJobs.get(row.job_id).corpus_state === 'complete' && rankedJobs.get(row.job_id).observed_keyword_count === row.keywords).length, acquired_unique_keywords: results.reduce((sum, row) => sum + row.keywords, 0), reviewed_headings: results.reduce((sum, row) => sum + row.headings, 0), page_states: tally(results, 'page_state', ['pass', 'fail', 'hold']), final_pass_queries: finalPassQueries, final_pass_percent_of_accepted: acceptedQueryCount ? Number((finalPassQueries / acceptedQueryCount * 100).toFixed(2)) : 0 }, non_claims: ['PASS is a minimum evidence conjunction, not a ranking guarantee.', 'Search-result features do not prove observed user transitions.', 'A held or failed page cannot lend its evidence to another article cluster.'] };
}

if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  const [rankedPath, headingPath, serpPath, semanticPath, reviewPath, ...packetPaths] = process.argv.slice(2); if (!reviewPath || !packetPaths.length) throw new Error('usage: node scripts/verify-routing-exact-page-review.mjs RANKED HEADINGS SERP SEMANTIC REVIEW PACKET...');
  const ranked = read(rankedPath); const headings = read(headingPath); const serp = read(serpPath); const semantic = read(semanticPath); const review = read(reviewPath); const packets = packetPaths.map(read);
  console.log(JSON.stringify(verifyRoutingExactPageReview({ ...ranked, digest: sha(ranked.bytes) }, { ...headings, digest: sha(headings.bytes) }, { ...serp, digest: sha(serp.bytes) }, { ...semantic, digest: sha(semantic.bytes) }, packets.map((packet) => ({ ...packet, digest: sha(packet.bytes) })), review.value), null, 2));
}

...
const signals=buildSerpActionSignals({tasks:[{task_id:"visual",group_id:"g1",keyword:"髪型",spell:null},{task_id:"book",group_id:"g2",keyword:"本",spell:{keyword:"書籍",type:"including_results_for"}},{task_id:"plain",group_id:"g3",keyword:"一般",spell:null}],features:[{feature_id:"visual:images:0",task_id:"visual",feature_type:"images",rank_absolute:3}],organicResults:[{task_id:"book",rank_absolute:2,price:{current:1000},rating:{value:4.5}},{task_id:"book",rank_absolute:4,price:null,rating:null}]});
assert.equal(signals.length,2);
const visual=signals.find((row)=>row.task_id==="visual"),book=signals.find((row)=>row.task_id==="book");
assert.deepEqual(visual.signal_types,["visual"]);assert.ok(visual.recommended_formats.includes("original_images"));assert.equal(visual.evidence[0].evidence_id,"visual:images:0");
...
console.log("SERP action signals: OK (evidence-bound format, commercial and spelling guidance)");

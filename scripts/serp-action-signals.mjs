...
export function buildSerpActionSignals({tasks,features,organicResults}){
  const featuresByTask=new Map(),organicByTask=new Map();
  ...
  for(const task of tasks){
    ...
    for(const feature of taskFeatures){const guidance=featureGuidance[feature.feature_type];if(!guidance)continue;signalTypes.push(guidance.signal);formats.push(...guidance.formats);titles.push(fill(guidance.title,task.keyword));headings.push(fill(guidance.heading,task.keyword));evidence.push({evidence_type:"serp_feature",evidence_id:feature.feature_id,feature_type:feature.feature_type,rank_absolute:feature.rank_absolute??null})}
    const priced=organic.filter((row)=>row.price!=null),rated=organic.filter((row)=>row.rating!=null);

...
const story={story_digest:'d',problem_clusters:['a','b','c'].map(id=>({id,answer_scope:id})),story_transitions:[{from_problem:'a',to_problem:'b',resolved_before_transition:'整理できた',next_question:'次は？',rationale:'理由'},{from_problem:'b',to_problem:'c',resolved_before_transition:'理解できた',next_question:'どう使う？',handoff_material:'判断条件と未確認事項',rationale:'別の回答が必要'}]};
...
assert(result.routes.every(r=>r.target_resolution==='article_candidate_resolved'));
assert.equal(result.routes[1].handoff_material,'判断条件と未確認事項');

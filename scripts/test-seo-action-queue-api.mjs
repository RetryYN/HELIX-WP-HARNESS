...
try{
  ...
  assert.equal(s.type_counts.primary_source_verification,63);assert.equal(s.type_counts.content_gap_review,13);assert.equal(s.type_counts.internal_link_review,17);assert.equal(s.type_counts.rank_monitor_registration_review,1);assert.equal(s.type_counts.semantic_sense_review,35);assert.equal(s.semantic_pending_task_count,144);
  const semanticRows=queue.rows.filter((row)=>row.action_type==="semantic_sense_review");assert.equal(new Set(semanticRows.flatMap((row)=>row.context.pending_task_ids)).size,144);assert.equal(semanticRows.reduce((sum,row)=>sum+row.context.pending_task_count,0),144);assert(semanticRows.every((row)=>row.context.decision_packet_digest===site.semantic_resolution_decision_packet.packet_digest&&!row.context.context_relevance_inferred&&!row.context.auto_approval&&row.evidence_references.length===row.context.pending_task_count));

...
try {
  ...
  assert.equal(row.comparison.observed_target_keyword_count,targetKeywords.size);assert.equal(row.comparison.duplicate_keyword_count,targetKeywords.size);assert.equal(row.comparison.duplicate_rate_target,1);assert.equal(row.comparison.jaccard_overlap_rate,1);assert.equal(row.comparison.competitor_unique_keyword_count,0);assert.equal(row.comparison.target_unique_keyword_count,0);assert.equal(row.comparison.scope.full_rank_database,false);assert.equal(row.comparison.policy,"observed-serp-domain-keyword-set.v2");assert.equal(row.scope.site_id,site.site_id);
  const taskIds=new Set(data.groups.filter((group)=>group.site_id===site.site_id).flatMap((group)=>group.task_ids)),pageUrl=new URL(`/api/v1/pages?site_id=${encodeURIComponent(site.site_id)}&limit=100`,'http://localhost'),pageResponse=routeResearchApi(pageUrl.pathname,pageUrl,data,db),expectedPageCount=new Set(data.serp_page_keyword_edges.filter((edge)=>taskIds.has(edge.task_id)).map((edge)=>edge.canonical_url)).size;assert.equal(pageResponse.status,200);assert.equal(pageResponse.body.meta.total,expectedPageCount);assert.equal(expectedPageCount,565);assert(pageResponse.body.data.every((item)=>item.canonical_url&&item.scope.site_id===site.site_id));

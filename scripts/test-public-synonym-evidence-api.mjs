...
try {
  assert.equal(
    Number(
      db.prepare("SELECT COUNT(*) count FROM public_lexical_sources").get()
        .count,
    ),
    1,
  );
  assert.equal(
    Number(
      db.prepare("SELECT COUNT(*) count FROM public_synonym_pairs").get().count,
    ),
    11753,
  );
  assert.equal(
    Number(
      db
        .prepare(
          "SELECT COUNT(*) count FROM public_synonym_pairs WHERE context_review_required=1 AND auto_replacement=0",
        )
        .get().count,
    ),
    11753,
  ...
  const data = projectDashboard(db),
    site = data.sites.find((row) => row.public_synonym_evidence?.rows.length);
  assert(site);
  ...
  assert(
    response.body.data.some(
      (row) => row.left_term === "トラブル" || row.right_term === "トラブル",
    ),
  );
  assert(
    response.body.data.every(
      (row) =>
        ...
        !row.auto_replacement &&
        row.evidence_digest.length === 64,
    ),
  );
  assert.equal(
    response.body.source.pair_file_sha256,
    "d8a017df64945559aa3c8f713efba96c52cada3055558135762f76c482339a9e",
  );
  ...
  const mcp = handleMcpMessage(
    {
      ...
      method: "tools/call",
      params: {
        name: "search_public_synonyms",
        arguments: { site_id: site.site_id, query: "トラブル", limit: 100 },
      },
    },
    data,
  ).result;
  ...
  assert.equal(mcp.structuredContent.automatic_replacement, false);
  console.log(
    `public synonym evidence API/MCP: OK (11,753 pairs, 9,487 terms, ${site.public_synonym_evidence.summary.site_corpus_matched_pair_count} corpus matches, $0)`,
  );
} finally {
  db.close();
}

...
const review=(quality_score,review_state="ready")=>({quality_score,review_state,evidence_count:1,character_count:30,oracle:{evidence_reference_resolved:true}}),groups=[{id:"g1",main_keyword:"IT 就活"},{id:"g2",main_keyword:"IT 面接"}],candidates=[
  {candidate_id:"t1",group_id:"g1",content_type:"title",text:"IT就活の企業選びと面接対策をわかりやすく解説",evidence_ids:["e1"],review:review(95)},
...
assert.equal(rows.length,2);assert.equal(rows[0].title_candidate_id,"t1");assert.equal(rows[0].metrics.shared_evidence_count,1);assert.equal(rows[0].metrics.title_evidence_coverage,1);assert.equal(rows[0].metrics.selected_heading_count,2);assert.equal(rows[0].auto_approval,false);assert.equal(rows[0].status,"proposed");assert.equal(rows[0].composition_digest.length,64);assert.equal(rows[1].review_state,"blocked");assert.ok(rows[1].issues.includes("title_unavailable")&&rows[1].issues.includes("outline_unavailable"));
console.log("content plan coherence: OK (deterministic title selection, evidence/lexical coverage, review state, no auto-approval)");

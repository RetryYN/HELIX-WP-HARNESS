...
export function buildKeywordBoundaryOracle(legacyRows,intentPairs){
  ...
  for(const key of actionableKeys){
    ...
    const sameGroup=intentEvidence?.current_same_group??urlEvidence.current_same_group,urlOverlap=urlEvidence?.overlap_ratio??0,intentScore=intentEvidence?.intent_similarity_score??null;
    ...
    const leftTaskId=intentEvidence?.left_task_id??urlEvidence.source_task_id,rightTaskId=intentEvidence?.right_task_id??urlEvidence.target_task_id,leftKeyword=intentEvidence?.left_keyword??urlEvidence.source_keyword,rightKeyword=intentEvidence?.right_keyword??urlEvidence.target_keyword,record={left_task_id:leftTaskId,right_task_id:rightTaskId,left_group_id:intentEvidence?.left_group_id??urlEvidence.source_group_id,right_group_id:intentEvidence?.right_group_id??urlEvidence.target_group_id,left_keyword:leftKeyword,right_keyword:rightKeyword,current_same_group:Boolean(sameGroup),url_overlap_ratio:urlOverlap,shared_url_count:urlEvidence?.shared_url_count??0,intent_similarity_score:intentScore,intent_components:intentEvidence?.components??null,decision,recommended_action:recommendedAction,reason_codes:reasonCodes,url_evidence_digest:urlEvidence?.evidence_digest??null,intent_evidence_digest:intentEvidence?.pair_digest??null,policy:"keyword-boundary-consensus.v1",review_required:true,auto_mutation:false};
    rows.push({...record,boundary_digest:digest(record)});
  }
  const priority={merge_consensus_review:0,split_consensus_review:1,merge_signal_conflict_review:2,url_only_merge_review:3,current_group_boundary_review:4,semantic_adjacency_review:5,internal_link_boundary_review:6};

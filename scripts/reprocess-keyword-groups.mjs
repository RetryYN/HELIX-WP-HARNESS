...
console.log(JSON.stringify({result:resultPath,actual_keywords:next.tasks.length,roots:hierarchy.filter((row)=>row.relation==="root").length,aliases:hierarchy.filter((row)=>row.relation==="reordered_alias").length,article_keyword_groups:articleKeywordGroups.length,unresolved_groups:articleKeywordGroups.filter((group)=>group.resolution_state==="unresolved").length},null,2));

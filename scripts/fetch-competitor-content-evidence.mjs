...
const rows=db.prepare(`SELECT d.group_id,o.task_id,o.rank_group,o.url,o.domain,s.domain AS own_domain FROM serp_organic_results o JOIN data_provider_b_tasks d ON d.task_id=o.task_id JOIN keyword_groups g ON g.group_id=d.group_id JOIN sites s ON s.site_id=g.site_id WHERE o.rank_group<=? ORDER BY o.rank_group,o.task_id`).all(maxRank);db.close();
const byUrl=new Map();

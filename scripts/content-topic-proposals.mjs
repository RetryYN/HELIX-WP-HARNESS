...
export function buildEvidenceBoundGenerationCandidates(groups,proposals,competitorTerms,{maxCompetitorHeadings=3}={}){
  ...
  for(const group of groups.filter((item)=>item.main_keyword)){
    ...
    if(topics.length){const titleEvidence=topics.slice(0,2).map((item)=>item.proposal_id),coverage={topic_count:Math.min(2,topics.length)};add("title",null,titleText,"serp_demand",titleEvidence,coverage,"demand_compound");const lead=topics[0].display_topic.replace(/[？?]+$/u,"");add("title",null,`${group.main_keyword}を解説｜${lead}`,"serp_demand",[topics[0].proposal_id],{topic_count:1,lead_topic_kind:topics[0].topic_kind},"demand_explainer")}

...
const topicAxis=(mainKeyword,topic)=>{const main=new Set(tokenizeMatchText(mainKeyword)),tokens=tokenizeMatchText(topic).filter((token)=>!main.has(token)&&!generic.has(token));return[...new Set(tokens)].join("・")};
...
export function buildEvidenceBoundGenerationCandidates(groups,proposals,competitorTerms,{maxCompetitorHeadings=3}={}){
  ...
  for(const group of groups.filter((item)=>item.main_keyword)){
    const topics=proposals.filter((item)=>item.group_id===group.id&&item.relation==="same_group").slice(0,8),primaryTitleAxes=[...new Set(topics.slice(0,2).map((item)=>topicAxis(group.main_keyword,item.display_topic)).filter(Boolean))],titleText=primaryTitleAxes.length?`${group.main_keyword}の${primaryTitleAxes.join("・")}をわかりやすく解説`:group.main_keyword;
    const add=(contentType,level,textValue,evidenceType,evidenceIds,coverage,variantKey)=>{const generation={variant_key:variantKey,generator_kind:"deterministic_rule",generator_version:"evidence-bound-generation.v2",input_digest:digest({group_id:group.id,main_keyword:group.main_keyword,evidence_type:evidenceType,evidence_ids:evidenceIds,coverage})};results.push({candidate_id:digest([group.id,contentType,level,textValue,evidenceType,evidenceIds,variantKey]).slice(0,24),group_id:group.id,content_type:contentType,heading_level:level,text:textValue,evidence_type:evidenceType,evidence_ids:evidenceIds,coverage,generation,status:"proposed",candidate_digest:digest([group.id,contentType,level,textValue,evidenceType,evidenceIds,coverage,generation])})};
    if(topics.length){const titleEvidence=topics.slice(0,2).map((item)=>item.proposal_id),coverage={topic_count:Math.min(2,topics.length),axes:primaryTitleAxes};add("title",null,titleText,"serp_demand",titleEvidence,coverage,"demand_compound");const leadAxis=topicAxis(group.main_keyword,topics[0].display_topic),leadText=leadAxis?`${group.main_keyword}とは？${leadAxis}をわかりやすく解説`:`${group.main_keyword}とは？検索ニーズを整理`;add("title",null,leadText,"serp_demand",[topics[0].proposal_id],{topic_count:1,lead_topic_kind:topics[0].topic_kind,axis:leadAxis},"demand_explainer")}
    topics.forEach((item,index)=>add("heading",index===0?2:3,item.display_topic,"serp_demand",[item.proposal_id],{occurrence_count:item.occurrence_count,task_count:item.task_count},item.topic_kind==="paa"?"question_topic":"related_topic"));

...
assert(html.includes("伏せ字URLは比較対象外（左 未集計件・右 未集計件）"));
const identityHtml=renderBlindSemanticReview({...packet,cases:[{...packet.cases[0],result_comparison:{shared_urls:["https://example.test/shared"],left_redacted_url_count:2,right_redacted_url_count:1}}]});
assert(identityHtml.includes("識別可能なURLの完全一致: 1件"));
assert(identityHtml.includes("伏せ字URLは比較対象外（左 2件・右 1件）"));

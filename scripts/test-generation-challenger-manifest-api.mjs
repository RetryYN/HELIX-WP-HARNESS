...
try{
  ...
  assert(manifest.requests.every((row)=>row.input.baseline_artifact_ids.length&&row.input.evidence_ids.length&&row.prompt_contract.natural_language_prompt_recorded&&row.prompt_contract.system_instruction&&row.prompt_contract.output_schema&&row.prompt_contract.output_schema_digest.length===64&&row.input_contract.estimated_maximum_input_tokens>0&&row.token_ceiling.maximum_output_tokens>0&&row.maximum_cost_usd==null&&!row.execution_authorized&&!row.external_generation_triggered));

...
try{
  ...
  assert(manifest.requests.every((row)=>row.input.baseline_artifact_ids.length&&row.input.evidence_ids.length&&row.prompt_contract.natural_language_prompt_recorded&&row.prompt_contract.output_schema_digest.length===64&&row.maximum_cost_usd==null&&!row.execution_authorized&&!row.external_generation_triggered));
  ...
  console.log("generation challenger manifest API/MCP: OK (303 requests, five capabilities, cumulative $5 gate, zero execution)");
}finally{db.close()}
